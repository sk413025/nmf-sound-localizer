...
class NMFTokenizer:
    ...
    def __call__(self, Y: np.ndarray, H: Optional[np.ndarray] = None) -> Tuple[List[str], np.ndarray]:
        ...
        logger.info("NMFTokenizer.__call__: Y shape=%s, W shape=%s, mode=%s, topk=%d", Y.shape,
                     None if self.W is None else self.W.shape, self.mode, self.topk)
        ...
        logger.info("NMFTokenizer.__call__: z shape=%s, s_hat shape=%s, emitted %d tokens", z.shape,
                     s_hat.shape, len(toks))
        return toks, s_hat
