...
def generate(data_root: Path, output_dir: Path) -> list[Path]:
    ...
    gs = gridspec.GridSpec(
        ...
        left=0.07, right=0.95, bottom=0.04, top=0.96,
    )

    # --- Row 1: Singular values (a) ---
    ax_sv = fig.add_subplot(gs[0, 1:5])
    r_idx = np.arange(1, len(S) + 1)
    line1, = ax_sv.semilogy(r_idx, S, marker="o", markersize=2, label=r"$\sigma_r$", color="C0")
    ax_sv.set_xlabel("Mode index r")
    ax_sv.set_ylabel(r"Singular value $\sigma_r$")

    ax_sv2 = ax_sv.twinx()
    line2, = ax_sv2.plot(r_idx, np.cumsum(energy_r), "g-", marker="s", markersize=2, label="Cum Energy")
    line3, = ax_sv2.plot(r_idx, np.cumsum(doa_cap_r_norm), "r-", marker="^", markersize=2, label="Cum DOA Cap")
    ...
    for r in range(n_modes):
        ...
        ax_f.plot(freqs_smooth, u_abs, color=color, linewidth=0.8)
        ...
        ax_f.grid(True, alpha=0.3, linewidth=0.3)
        ...
        ax_p.plot(angles_rad, v_norm, color=color, linewidth=0.8)
        ax_p.fill(angles_rad, v_norm, color=color, alpha=0.15)
        ax_p.grid(True, alpha=0.3, linewidth=0.3)
