...
## Abstract
After object-specific calibration over a 37-angle half-plane benchmark (0-180° in 5° steps), a single non-contact laser Doppler vibrometer (LDV) measurement point on an acrylic plate yielded repeatable direction-dependent vibration fingerprints. These spectra occupied a compact angle-ordered structure, with neighboring calibrated directions remaining locally similar. Replacing the calibration probe with held-out speech weakened that structure but did not erase it: within-angle similarity remained higher than between-angle similarity across the benchmark. The main difficulty therefore arose when nearby calibrated directions had to be separated, because speech broadened local overlap and destabilized immediate one-angle commitment. A guided solver that pools nearby evidence before committing to one direction gave the strongest readout in this benchmark. Across an exploratory five-object screen, structured directional fingerprints and low-rank continuity recurred under matched object-specific calibration, but the data support recurrence only within this small sample rather than a transfer law across objects. Together, these results show that calibrated single-point vibrometric readout of sound direction is feasible for this acrylic-plate benchmark, and that reliable decoding depends on how local overlap among nearby calibrated directions is handled.

## Introduction
Sound direction is usually inferred from spatial differences across sensors. Multi-microphone methods such as beamforming, MUSIC, and ESPRIT extract direction from those differences and remain central to acoustic localization [@capon1969fkw; @schmidt1986music; @roy1989esprit; @krim1996array; @vantrees2002optimum; @johnson1993array; @brandstein2001microphone_arrays; @benesty2008microphone_array]. Yet direction can also change how incident sound couples into an object's vibration field. That observation motivates the question studied here: after object-specific calibration, can one fixed vibrometric readout from an acrylic plate serve as a reliable directional measurement?
...
A shared 70° exemplar makes one local mechanism visible. The initial physical match is broad across nearby directions, but one guided update narrows that support around the correct local neighborhood and leaves less unexplained signal [@gregor2010lista; @monga2021unrolling]. The compact callouts summarize the same shift numerically: the fraction of the match that falls within 15° of the true direction rises from 0.18 to 0.98, while the remaining residual falls from 1.00 to 0.48. In the clean-condition comparison, the decoder family that performs this local pooling also gives the highest accuracy in this restricted view.
...
We next test whether the same local-overlap handling matters across the full held-out benchmark. The measured template matrix \(H\) retains a banded correlation structure, and the guided neighborhood map places most of its weight along that same local angle ordering (Fig. 5c). The prediction summaries follow the same pattern: outputs stay most concentrated near the correct neighborhood when nearby evidence is pooled before commitment, both in the confusion-family block and in representative conditional profiles (Fig. 5b,d). The same consequence appears across noise levels and across all 37 angles: the decoder family that pools nearby evidence before commitment remains strongest, whereas immediate or weakly constrained choices degrade more sharply (Fig. 5a,e). In this benchmark, reliable readout therefore depends on resolving local overlap before committing to a single calibrated direction.
...
e, Per-angle decoder accuracy: five-seed clean mean \(P(\mathrm{correct})\) across the 37 measured angles, shown as a 3-angle centered moving-average display with light \(\pm 1\) s.e.m. shading, comparing the guided solver, router-bypass, OMP baseline, and dense routing. The family that pools nearby evidence before commitment remains strongest overall, whereas dense routing remains near chance across almost the entire angle set.
...
## Discussion
Object-specific calibration turns the acrylic plate's direction-dependent vibration response into a readable single-point measurement pathway in this benchmark. At one fixed LDV position, the plate produces repeatable directional fingerprints, those fingerprints occupy a compact local structure across calibrated angles, and held-out speech remains readable when decoding respects overlap among nearby directions [@gigan2022imaging_computing_disorder; @rotter2017complex_media]. The main conclusion is therefore bounded but clear: for this acrylic-plate benchmark, sound direction can be read from one calibrated vibrometric measurement, provided the decoder handles local neighborhood overlap rather than forcing an immediate single-angle choice.
