...
Our working hypothesis is that direction changes how sound couples into an object's vibration modes, and that this change can be read as a repeatable spectrum at one fixed point. Incident sound first produces an object response, the LDV records that response as a measured fingerprint, object-specific calibration organizes those fingerprints into a template matrix \(H\), and a decoder must then read that calibrated matrix when nearby directions overlap. We test that pathway in a controlled benchmark and ask two linked questions: does the calibrated response occupy a compact measured structure, and can a decoder read it reliably once source content changes?
...
### A five-object screen tests recurrence under matched calibration (Fig. 6)
We finally ask whether matched calibration reveals the same response-to-readout structure in a small five-object screen. Each object yields structured angle-frequency fingerprints in its own calibrated template matrix \(H\) (Fig. 6a,b). Applying the same centered-magnitude SVD view used in Fig. 2 again shows early energy capture across the five objects (Fig. 6c). The bounded conclusion is therefore recurrence beyond the acrylic plate under matched object-specific calibration, not transfer from one object to another.

The same screen also shows why the claim should remain narrow. Each object remains above chance, but normalized response energy does not monotonically track Top-1 accuracy across the five objects (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e). This five-object screen therefore supports recurrence under matched calibration, not a material-agnostic transfer law.
