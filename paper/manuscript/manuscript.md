...
Figure 3 sets that readout requirement in operational form: several nearby angles remain plausible before subtraction begins, so local evidence must be preserved long enough to resolve their overlap. To test that requirement, we use a physics-guided solver that scores the residual against the calibrated dictionary, concentrates the match within a local angle neighborhood, and only then subtracts [@gregor2010lista; @monga2021unrolling].
...
### Matched calibration reveals object-conditioned directional structure beyond the acrylic reference object (Fig. 6)
The acrylic plate is the reference object used to dissect the local-overlap problem. The remaining question is narrower: does matched calibration reveal organized directional structure only in that plate, or does the same kind of object-conditioned pattern also appear in other passive objects? Across five ordinary objects, each object again yields an angle-frequency fingerprint matrix \(H\) with visible directional structure (Fig. 6a,b). Each object also remains above chance under its own matched calibration (Fig. 6d), and the compactness summary in Fig. 6c is consistent with the same kind of organized fingerprint space. The informative band, however, shifts across objects (Fig. 6e), so the extension is not one fixed spectral template reused everywhere.

That extension remains bounded. Figure 6 does not test whether one decoder trained on the acrylic plate transfers across objects, nor whether local-routing dynamics recur identically in each object. It tests a narrower point: after object-specific matched calibration, structured directional fingerprints and above-chance readout are not unique to the acrylic plate. Response energy, informative band, and Top-1 accuracy still differ across objects, so each object carries its own directional code and still requires its own calibration.
...
The five-object comparison extends that story in one bounded way. It does not show that one decoder or one spectral template transfers across objects. Instead, it shows that after each object is calibrated on its own, structured directional fingerprints and above-chance readout are not unique to the acrylic plate. Normalized response energy, informative bands, and Top-1 performance still differ across objects, which indicates that each object carries its own directional code and still requires object-specific calibration. The compactness summary in Fig. 6c is consistent with that extension, but the cross-object variation in readout and band placement keeps the conclusion object-conditioned and bounded.
