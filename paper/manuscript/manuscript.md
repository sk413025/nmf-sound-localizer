...
The guided solver preserves the measured local organization at the prediction level. The calibrated template matrix \(H\) shows a near-diagonal band of local overlap, and the guided neighborhood-emphasis map reproduces that coarse angle ordering rather than dispersing activity broadly across the grid (Fig. 5c).

That preserved local organization shapes the predictions themselves. In the confusion displays and angle-specific profiles, guided decoding remains concentrated near the correct neighborhood, whereas router-bypass spreads more broadly, the OMP baseline fractures into off-diagonal errors after early commitment, and dense routing collapses toward a preferred output mode (Fig. 5b,d).

The same delayed-commitment logic also stabilizes performance summaries. Across noise levels and across clean per-angle accuracy, the guided solver remains more stable than the more immediate or more diffuse alternatives (Fig. 5a,e). Delaying commitment until local overlap has narrowed therefore improves both the structure of the prediction and the downstream decoding outcome.
