...
Figure 4 makes that requirement visible at the first update. The residual first forms a broad local match over nearby grouped templates. Routing then concentrates that match within the locally plausible band, so the update stays inside the measured neighborhood before subtraction acts. On the same local angle frame, Fig. 4a shows that admissible behavior means contracting that broad match back into the measured neighborhood rather than sharpening outside it. Figure 4b shows the same rule on one representative 70° validation clip: the learned cue and routed weight stay inside the broad physical support and carve a narrow local update from within it. Figure 4c summarizes the immediate first-step consequence at exact, 5°, 10°, and 15° thresholds: exact commitment rises sharply after one guided step, while local commitment inside the operative neighborhood becomes nearly saturated.

That contraction is not anecdotal. Validation-wide cumulative mass-within-radius curves shift sharply inward after the first guided step (Fig. 4d). At the operative 15° neighborhood the same gain remains positive across the directional grid (Fig. 4e), and the clip-level gain CDF shows that this inward shift is positive for nearly all validation clips rather than being driven by a small subset (Fig. 4f). The admissible readout is therefore the one that preserves and contracts the broadened neighborhood exposed in Fig. 3 before subtraction is allowed to act. The next question is whether that same radius-based neighborhood metric survives all the way to final prediction and still separates decoder families. The full neighborhood-preserving formulation is given in Supplementary Methods 4.

![](../figures/fig04_solver-dynamics.jpg)

**Fig. 4 | Preserving the broadened neighborhood keeps subtraction admissible.**
a, Admissibility synthesis. Validation-mean broad support, the contracted first-step profile, and one representative 70° validation clip are shown on the same local angle frame. The measured neighborhood appears only as the physical reference band that the guided update must respect.
...
c, First-step operating-point recovery. Before-versus-after mass is summarized at exact, 5°, 10°, and 15° operating thresholds. The first guided step sharply recovers exact commitment while keeping local commitment inside the operative neighborhood nearly saturated.
