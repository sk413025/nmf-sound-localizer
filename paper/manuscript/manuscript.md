...
Here we show that ordinary passive objects already carry directional structure in their vibrations that can be read from one fixed laser spot.
In an acrylic plate, that structure appears as a compact local code whose neighboring directions reuse shared response patterns rather than fragmenting into unrelated templates.
Held-out speech does not erase that code, but turns it into a neighborhood ambiguity problem because adjacent directions remain partially shared.
Readout therefore fails when it commits to one angle before evidence shared across that neighborhood has been consolidated and subtraction can sharpen it.
Across acrylic, paper, cardboard, wood, and a laptop shell, the same locally ordered directional code recurs.
That recurrence identifies ordinary passive objects as a class of directional sensing substrates: directional information need not reside only in dedicated hardware, but can already be embedded in the structures present in a scene.
...
The acrylic plate is the clearest first revealer of that behavior, but it is not the endpoint of the paper. The broader question is whether the same local directional organization recurs across ordinary passive objects, and whether local separability rather than response energy decides when that code remains readable. This question sits alongside wave systems that treat complex scattering as an encoding resource rather than as noise [@fink1997time; @duarte2008single; @gigan2022imaging_computing_disorder; @rotter2017complex_media; @jiang2020randomized_metamaterial]. The central questions are therefore whether ordinary passive objects share a recurring local directional code, and whether local separability is the variable that decides when that code remains readable. If so, part of directional sensing would move from dedicated hardware into the passive structures already present in a scene [@iravantchi2023sawsense; @wang2024gpms].
...
Different frequency bands emphasize different directional sectors (Fig. 1e), indicating that multiple dispersive vibration pathways contribute with angle-dependent weights [@rotter2017complex_media; @kuttruff2025room_acoustics]. That establishes the code as structured rather than accidental. The next question is whether it also traces a readable geometry, or whether it is only a collection of different spectra.
...
In the full angle-frequency matrix, neighboring directions reuse related structure (Fig. 2d-f), and the same early low-rank regime already captures most of the centered-magnitude energy (Fig. 2a-c): six components account for 80.3% and eight account for 85.1%.

That pattern has a simple consequence. Changing direction mainly reweights a limited set of structural responses instead of generating fully independent fingerprints at every angle. Nearby directions remain related without becoming interchangeable. The directional code is therefore compact and locally ordered before any decoder enters the story.
...
f, Inter-angle fingerprint similarity matrix of \(H\). The near-diagonal high-similarity band shows that nearby angles remain related rather than interchangeable. Inset, a 2D embedding gives the same point in one view: the measured angle trajectory stays curved and locally ordered rather than fragmenting into isolated templates.
...
Figures 4 and 5 answer the next bounded question: once local overlap is the governing difficulty, what readout remains scientifically admissible? The answer is narrow. Subtraction helps only after the measured neighborhood has been preserved [@gregor2010lista; @monga2021unrolling].

Figure 4 shows that requirement directly against measured structure. In a representative 70° validation clip, broad matching initially excites several nearby calibrated angles above the physically plausible band in \(H\) (Fig. 4a,b), and the local gate then contracts support around the correct neighborhood (Fig. 4c). After the first guided step, the residual drops from 1.00 to 0.48 while the fraction of update mass within 15° rises from 0.18 to 0.98 (Fig. 4d). The correction is not merely smaller. It has collapsed into the neighborhood that the measurement itself says is plausible. That is why guided decoding stays highest under clean evaluation: it preserves that neighborhood before subtraction, whereas router-bypass, OMP, and dense routing progressively break the measured local order (Fig. 4e).
...
Figure 5 asks the remaining admissibility question: does that readout stay aligned with the measured neighborhood it exploits? It does. The key point is that the learned geometry stays anchored to the measured neighborhood rather than inventing a different structure. On the shared angle frame, the calibrated template matrix \(H\) and the guided neighborhood-emphasis map show the same coarse near-diagonal organization (Fig. 5d,e). That agreement is quantitative as well as visual: matrix-level alignment reaches \(r = 0.47\), the per-angle local-band summaries reach \(r = 0.46\), and both exceed the corresponding 95th-percentile permutation nulls (Fig. 5f).

Clean evaluation reaches the same conclusion. Guided decoding stays near the diagonal while the OMP baseline fractures into broader off-diagonal errors after committing too early, and delayed commitment remains the strongest compared rule across angle and SNR (Fig. 5a-c). The admissible readout therefore follows the measured neighborhood it exploits.

If the code were only a quirk of one flat acrylic plate, the story would stop here as a single-object curiosity. It does not.
...
Across five ordinary passive objects—an acrylic plate, paper cup, cardboard box, wooden board, and laptop shell—the same locally ordered directional code reappears. Each object shows structured angle-frequency fingerprints, a finite neighborhood of positive local ordering, and above-chance single-point readout under matched calibration (Fig. 6a-c). The directional organization revealed in acrylic is therefore a broader passive-object phenomenon.

That recurrence sets up the next question: what governs how usable that code is from object to object? Matched object-specific calibration keeps Top-1 readout above chance across the full set (Fig. 6d). Across objects, mean Top-1 aligns more closely with mean top-3 overlap burden than with normalized overall \(|H|\) energy alone. The paper cup provides the clearest contrast: it carries the strongest overall energy, yet its neighboring angles overlap more broadly, so it reads out worse than cardboard. Local separability therefore matters more than raw energy for directional decoding.

The same logic carries into frequency. A shared contrast-selection rule identifies a different informative window for each object, and the corresponding band-limited directional codes differ across those object-specific bands (Fig. 6e). What recurs across the tested set is not one universal spectrum. It is one local directional principle expressed through object-specific informative bands. Recurrence across materials therefore identifies a class of ordinary passive directional substrates rather than a special property of one acrylic plate. The directional code is located in the objects themselves, not in purpose-built directional hardware alone.
...
## Discussion
Directional encoding need not be designed into arrays and specialized sensors alone. Across the tested objects, passive structural vibration itself instantiates a finite local code that one fixed vibrometric point can read after matched calibration. Passive structure is therefore part of the sensing substrate, not merely a nuisance around it.
...
Placed in context, this view links the paper to several literatures. Studies of engineered disorder and metasurfaces treat complex scattering as a computational wave encoder [@jiang2020randomized_metamaterial; @hoang2021single_pixel_doa]. Single-sensor localization studies show that direction-dependent transfer structure can be exploited without large arrays [@elbadawy2018lego_doa; @dipassio2023doa_single_sensor; @rutowski2024reverb_single_sensor; @rutowski2025_structural_beamforming]. Here the same logic appears in unmodified passive objects rather than in engineered structures or specialized sensor hardware.

The broader implication is therefore not merely that ordinary objects can be decoded. It is that recurrence across ordinary objects identifies passive structure itself as part of the directional sensing substrate. If a recurring local code can be read under matched calibration, then directional front-end information need not originate only in dedicated directional hardware. It can already reside in existing surfaces and structures [@iravantchi2023sawsense; @wang2024gpms]. The present results do not establish a deployable ambient system, but they do change the design question: rather than asking only where to place directional sensors, we can also ask which passive structures in a scene already carry usable directional information.

That substrate shift also suggests one bounded downstream consequence. Once coarse directional information is available at the substrate layer, a system may be able to decide which part of a scene to process next without first solving full localization. This is only a front-end filtering role for coarse directional cues, not a claim of full scene understanding.
