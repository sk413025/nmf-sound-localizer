...
## Abstract
Matched calibration over a 37-angle half-plane benchmark (0-180° in 5° steps) shows that a passive acrylic plate preserves sound-direction information in a single non-contact laser Doppler vibrometer (LDV) readout. The measured vibration fingerprints are repeatable and occupy a compact angle-ordered structure, with neighboring calibrated directions remaining locally similar. Replacing the white-noise calibration probe with held-out speech weakens this structure but does not erase it: within-angle similarity remains higher than between-angle similarity across the benchmark. The main readout difficulty therefore arises when nearby calibrated directions overlap, because speech broadens that local ambiguity and destabilizes immediate one-angle commitment. A physics-guided neighborhood-aware solver that keeps the calibrated dictionary fixed while pooling nearby evidence before final commitment tracks that measured local structure best across the benchmark. In an exploratory five-object screen, that matched-calibration sensing pathway recurs across a small object set under object-specific calibration, but each object retains its own directional code and informative band, so the pathway remains object-conditioned rather than transferable. These results show that calibrated single-point vibrometric direction readout is possible in this acrylic-plate benchmark, identify local-overlap handling as the operational constraint, and indicate that the same sensing pathway is not unique to the acrylic plate within this bounded screen.
...
Noise worsens the same failure mode. Greedy accuracy declines monotonically as additive noise increases in both the white-noise and speech conditions (Fig. 3f). Speech does not remove directional information from measured fingerprint space, but it broadens overlap among neighboring calibrated templates. The next step is therefore to ask what kind of readout can preserve that measured local neighborhood long enough to make a stable directional decision. Figure 4 tests whether a physics-guided solver can keep the calibrated dictionary fixed, pool nearby evidence, and delay final commitment until the correct neighborhood has been isolated.
...
### The physics-guided solver concentrates nearby overlap into the correct neighborhood (Fig. 4)
The solver's job is therefore to turn the broadened local overlap from Fig. 3 into a stable neighborhood-level decision before a final single-angle choice. A representative 70° case shows that behavior directly: the physics-guided solver concentrates nearby overlap into the correct local neighborhood, and less unexplained signal remains after one refinement step [@gregor2010lista; @monga2021unrolling]. The compact callouts summarize the same shift numerically: the fraction of the match that falls within 15° of the true direction rises from 0.18 to 0.98, while the remaining residual falls from 1.00 to 0.48.

The family comparison in Fig. 4d then gives the paper-level readout consequence of that local concentration step. In the same restricted clean five-seed sweep, the guided solver remains highest (0.98), router-bypass drops to an intermediate level (0.58), the OMP baseline drops further (0.44), and dense routing stays near chance (0.03). Placed alongside panels b and c, that clean ranking shows that the family carrying the local-concentration step is also the most accurate.
...
### Guided neighborhood emphasis tracks the measured local structure across the benchmark (Fig. 5)
Across this benchmark, the calibrated template matrix \(H\) retains a near-diagonal band of local overlap. In Fig. 5c, the measured local structure and the neighborhood-emphasis map follow that same band, indicating that the guided solver concentrates support where the calibrated fingerprints remain locally similar.

That local match is also visible in behavior across the four readout families. The row-normalized comparison block and the representative angle-specific profiles show that the guided solver keeps predictions concentrated near the correct neighborhood, whereas the OMP baseline shows fractured off-diagonal errors, router-bypass remains broader around the target neighborhood, and dense routing collapses toward a preferred output mode instead of maintaining angle-conditioned neighborhoods (Fig. 5b,d).

Across noise levels and all 37 angles, the family that best preserves the measured local structure also remains the most accurate within this benchmark, whereas the more immediate or more diffuse alternatives degrade more sharply (Fig. 5a,e). These benchmark results therefore support local evidence pooling before final commitment once nearby template overlap broadens.
...
### Matched calibration recurs across a bounded five-object screen (Fig. 6)
Across a small five-object screen, the matched-calibration sensing pathway is not unique to the acrylic plate. Each object yields its own structured angle-frequency fingerprint pattern in the calibrated template matrix \(H\) (Fig. 6a,b), and all five objects remain above chance under matched object-specific calibration despite clear differences in screening performance (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e), indicating that this pathway recurs through object-specific response structure rather than through one fixed spectral pattern. The centered-magnitude SVD summary in Fig. 6c provides a supporting compactness comparison across the same screen.

This consequence remains bounded. Within this five-object screen, matched calibration can be re-established across different objects, but normalized response energy does not monotonically track Top-1 accuracy (Fig. 6d), and the informative band does not stay fixed across objects (Fig. 6e). The result therefore supports recurrence of the sensing pathway under object-specific calibration, not transfer across objects or a material-agnostic law.
...
## Discussion
Matched calibration turns the acrylic plate's direction-dependent response into a calibrated single-point sensing pathway that remains readable under source change. At one fixed LDV position, the plate produces repeatable measured fingerprints, those fingerprints organize into a compact template matrix \(H\), and held-out speech preserves their local angle ordering even as it broadens overlap among neighboring calibrated directions [@gigan2022imaging_computing_disorder; @rotter2017complex_media]. In that setting, the operational readout problem is not whether directional structure remains, but how to resolve overlap among nearby calibrated directions without destroying that local ordering. The evidence therefore supports a bounded conclusion: in this acrylic-plate benchmark, sound direction can be read from one calibrated vibrometric measurement when the solver pools local evidence before committing to a single calibrated direction.

The five-object screen gives that conclusion a bounded broader consequence. The matched-calibration sensing pathway recurs across a small object set rather than appearing only in the acrylic plate: under matched object-specific calibration, each object yields structured fingerprints and above-chance screening. At the same time, normalized response energy, informative bands, and Top-1 performance differ across objects, which indicates that each object carries its own directional code and still requires object-specific calibration. The compactness summary in Fig. 6c is consistent with that recurrence, but the cross-object variation in screening and band placement keeps the conclusion object-conditioned and bounded.
...
Figures 4 and 5 compare four stable readout families that share the same matched-calibration dictionary but differ in how they handle overlap among nearby calibrated directions: *guided solver* first concentrates support within the locally matching neighborhood, *router-bypass* keeps the same staged updates without that concentration, *OMP baseline* makes immediate single-angle matches, and *dense routing* spreads weight broadly across angles. Figure 3 reports the correlation-based greedy diagnostic separately because immediate single-choice matching already degrades under content variation before full residual correction is applied.
