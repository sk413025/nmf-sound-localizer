...
Noise worsens the same failure mode. Greedy accuracy declines monotonically as additive noise increases in both the white-noise and speech conditions (Fig. 3f). Speech does not remove directional information from measured fingerprint space, but it broadens overlap among neighboring calibrated templates. The next test therefore asks whether a physics-guided neighborhood-aware solver can keep the calibrated dictionary fixed, pool that nearby evidence, and avoid an unstable first hard commitment.
...
Across this benchmark, the calibrated template matrix \(H\) retains a near-diagonal band of local overlap. In Fig. 5c, the Measured local structure and the Neighborhood-emphasis map follow that same band, indicating that the guided solver concentrates weight where the calibrated fingerprints remain locally similar.

That alignment becomes behaviorally visible across the benchmark. The row-normalized comparison block and the representative angle-specific profiles show that the guided solver keeps predictions concentrated near the correct neighborhood, whereas the OMP baseline produces fractured off-diagonal leakage from immediate hard commitment, router-bypass spreads more broadly around the target neighborhood, and dense routing collapses toward a preferred output mode rather than tracking angle-conditioned neighborhoods (Fig. 5b,d). Within this benchmark, the OMP baseline makes an immediate single-choice match on the calibrated dictionary, the guided solver uses the same residual-correction scaffold with neighborhood-aware local gating, router-bypass keeps that scaffold without the learned gate, and dense routing distributes evidence broadly across angles.

Across noise levels and all 37 angles, the guided solver remains the most accurate within this benchmark, whereas the immediate or weakly localized alternatives degrade more sharply (Fig. 5a,e). These benchmark results therefore support a physics-guided neighborhood-aware solver that pools local evidence before final commitment once nearby template overlap broadens.
...
### Matched calibration can be re-instantiated across a bounded five-object screen (Fig. 6)
Across a small five-object screen, the same matched-calibration pathway can be re-instantiated beyond the acrylic plate. Each object yields its own angle-frequency fingerprint pattern in the calibrated template matrix \(H\) (Fig. 6a,b), and all five objects remain above chance under matched object-specific calibration despite clear differences in screening performance (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e), indicating that the directional code recurs through object-specific response structure rather than through one fixed spectral pattern. The centered-magnitude SVD summary in Fig. 6c provides a supporting compactness comparison across the same screen.

The same screen also bounds that workflow relevance. Normalized response energy does not monotonically track Top-1 accuracy across the five objects (Fig. 6d), and the informative band shifts across objects (Fig. 6e). Matched calibration can therefore be repeated across a small object screen, but each object carries its own directional code and still requires object-specific calibration. This five-object screen does not establish transfer across objects or a material-agnostic law.
...
## Discussion
Matched calibration turns the acrylic plate's direction-dependent response into a calibrated single-point sensing pathway that remains readable under source change. At one fixed LDV position, the plate produces repeatable measured fingerprints, those fingerprints organize into a compact template matrix \(H\), and held-out speech preserves their local angle ordering even as it broadens overlap among neighboring calibrated directions [@gigan2022imaging_computing_disorder; @rotter2017complex_media]. The evidence therefore supports a bounded conclusion: in this acrylic-plate benchmark, sound direction can be read from one calibrated vibrometric measurement when a physics-guided readout pools local evidence before committing to a single calibrated direction.

The five-object screen shows that the matched-calibration pathway can be re-instantiated across a small object set rather than only in the acrylic plate. Under matched object-specific calibration, each object yields structured fingerprints and above-chance screening. At the same time, normalized response energy, informative bands, and Top-1 performance differ across objects, which indicates that each object carries its own directional code and still requires object-specific calibration. The compactness summary in Fig. 6c is consistent with that interpretation, but the cross-object variation in screening and band placement keeps the conclusion object-conditioned and bounded.
