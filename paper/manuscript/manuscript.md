...
## Abstract
We measured the vibration response of an acrylic plate with a single non-contact laser Doppler vibrometer (LDV) point after object-specific calibration over a 37-angle half-plane benchmark (0-180° in 5° steps). The measured LDV spectra yielded repeatable direction-dependent fingerprints that occupied a compact angle-ordered structure, with strong local similarity across neighboring calibrated directions. When the calibration probe was replaced by held-out speech, this structure weakened but remained present: within-angle similarity stayed higher than between-angle similarity across the benchmark. The main failure therefore appeared at the first hard choice rather than in the fingerprints themselves, because speech broadened overlap among nearby calibrated reference templates and destabilized immediate one-angle commitment. Among the tested decoders, the strongest readout came from a guided solver that pools nearby evidence before committing to one direction, and this was the strongest strategy within the tested benchmark. Across an exploratory five-object screen, structured directional fingerprints and low-rank continuity recurred under matched object-specific calibration, but the data support recurrence within this small sample rather than a transfer law across objects. Together, these results show that calibrated single-point vibrometric readout of sound direction is feasible in this benchmark, while reliable decoding depends on how local overlap among nearby calibrated directions is handled.

## Introduction
Acoustic direction sensing usually works by comparing what arrives at multiple microphones. That strategy underlies beamforming, MUSIC, ESPRIT, and related array-processing methods [@capon1969fkw; @schmidt1986music; @roy1989esprit; @krim1996array; @vantrees2002optimum; @johnson1993array; @brandstein2001microphone_arrays; @benesty2008microphone_array]. It is powerful, but it ties performance to aperture size and sensor placement, which can be difficult to accommodate in wearables, structural monitoring systems, and other compact or harsh settings [@grumiaux2022_ssl_survey_deep_learning]. We therefore ask whether directional readout might be possible from a calibrated object-plus-readout pathway, using one measurement point rather than a spatial microphone array.

Our working hypothesis is that direction changes how sound couples into an object's vibration modes, and that this change can be read as a repeatable spectrum at one fixed point. Incident sound excites direction-dependent plate response, object-specific calibration records that response across angles, and a single-point vibration readout samples the resulting fingerprint. We test that pathway in a controlled benchmark and ask two linked questions: does the calibrated response occupy a compact measured structure, and can a decoder read it reliably once source content changes?

Several lines of prior work motivate this possibility, but they do not yet show that ordinary objects provide a reproducible calibrated measurement pathway. Single-sensor studies have reported direction-dependent transfer functions in engineered scatterers and in structure-borne measurements [@elbadawy2018lego_doa; @dipassio2022_audio_capture_structural_sensors; @dipassio2023_waspaa_wake_word; @dipassio2023doa_single_sensor; @rutowski2024reverb_single_sensor; @rutowski2025_structural_beamforming]. Sparse reconstruction offers one useful language for inference once a calibrated angle grid has been defined [@malioutov2005sparse_doa; @donoho2006compressed_sensing; @candes2006robust_uncertainty; @candes2008compressive_sampling; @chen2001basis_pursuit; @tibshirani1996lasso; @mallat1993matching_pursuits; @pati1993omp; @tropp2007omp]. Array-free localization is also possible with acoustic vector sensors, but those require specialized collocated pressure-particle-velocity instrumentation [@nehorai1994vector_sensor]. What remains unresolved is whether everyday targets themselves can supply repeatable directional fingerprints that are systematic enough to calibrate, compress, and decode.
...
This low-rank observation gives a simple consequence for readout. Once calibration has mapped the measured response onto a discrete angle grid, ambiguity among nearby directions should be local rather than global. We therefore next test whether held-out speech preserves that local structure, and if so, what kind of decoder can still read it reliably.
...
The failure appears at the first hard choice. The correlation-based greedy diagnostic performs well on most white-noise fingerprints but drops to near chance on held-out speech clips (Fig. 3d). The pairwise similarity map explains why: white noise yields a near-identity pattern, whereas speech spreads similarity across neighboring calibrated angles while retaining local ordering (Fig. 3e). Once several nearby reference templates become jointly plausible, immediate one-angle commitment becomes unstable even though directional structure is still present.

Noise worsens the same failure mode. Greedy accuracy declines monotonically as additive noise increases in both the white-noise and speech conditions (Fig. 3f). Figure 3 therefore defines the decoder problem carried forward through the rest of the paper: directional information remains in measured fingerprint space, but readout becomes unreliable if the solver commits too early to a single calibrated angle.
...
### One local update concentrates nearby support and reduces the residual (Fig. 4)
When nearby calibrated directions overlap, one local update can sharpen the readout problem rather than solve it all at once. Figure 4 illustrates that bounded point: a broad match across nearby directions can be focused into one local neighborhood, and the unexplained signal is smaller after that step [@gregor2010lista; @monga2021unrolling].

The shared 70° exemplar makes that change concrete. Panels b and c place the initial broad match, the local gate, and the resulting local update on one axis, then show that less signal remains near 70° after the step. The compact callouts summarize the same change numerically: the fraction of the match that falls within 15° of the true direction rises from 0.18 to 0.98, while the remaining residual falls from 1.00 to 0.48.

The clean-condition comparison in panel d points in the same direction. In this restricted view, the guided solver ranks highest, router-bypass and the OMP baseline trail, and dense routing performs worst. The same ordering then persists across the full benchmark in Fig. 5.
...
b, Shared 70° exemplar: the broad match is concentrated into one local neighborhood around 70°.
...
### Neighborhood-aware readout is strongest across the benchmark (Fig. 5)
We then ask whether pooling nearby evidence before commitment remains advantageous across the full held-out benchmark. The measured template matrix \(H\) has a banded correlation pattern because nearby angles share related fingerprints, and the guided neighborhood map places most of its weight along that same local ordering (Fig. 5c). Readout should therefore be strongest when the decoder follows this local structure rather than forcing an immediate single-angle choice.

That contrast appears directly in the prediction summaries. In the confusion-family block, the guided solver stays closest to the diagonal, whereas the decoders that do not handle local overlap as effectively spread farther off axis or collapse toward preferred outputs (Fig. 5b). At representative directions, the guided solver also produces tighter local output profiles than router-bypass (Fig. 5d).

That advantage persists across the full benchmark. Across signal-to-noise ratio levels, the guided solver remains the strongest of the four tested decoder families, while router-bypass and the OMP baseline degrade more sharply and dense routing remains near chance (Fig. 5a). Across all 37 angles, the same ranking persists in the clean-condition accuracy summary (Fig. 5e). In this benchmark, readout is therefore strongest when nearby evidence is pooled before commitment, and the guided solver is the strongest of the tested ways to do that.
...
Object-specific calibration also yields structured angle-frequency fingerprints in five structurally different objects (Fig. 6a,b). Applying the same centered-magnitude SVD view used in Fig. 2 to each object's template matrix again shows early energy capture (Fig. 6c). Within this sample, both the directional encoding and its compact local continuity recur beyond the acrylic plate.
...
## Discussion
After object-specific calibration, sound direction becomes readable from a single LDV measurement because the measured vibration spectrum changes systematically with incidence angle. The main result is therefore not that an everyday object generically replaces a microphone array. It is that, in this controlled benchmark, a calibrated acrylic-plate readout can produce repeatable directional fingerprints that a single measurement point can sample [@gigan2022imaging_computing_disorder; @rotter2017complex_media].

For the acrylic plate, those fingerprints occupy a compact angle-ordered space. Held-out speech weakens but does not erase that geometry. The practical problem then becomes local overlap among nearby calibrated directions: immediate first-match decoding fails, whereas pooling evidence locally yields more reliable readout. Among the tested decoder families, the guided solver handles that overlap best. The five-object screen extends this picture only cautiously. Structured directional fingerprints and low-rank continuity recur within this small matched-calibration sample, but the present data do not establish a transfer law across arbitrary objects or mounting conditions.

These findings connect to, and differ from, several existing lines of work. Prior studies used engineered disorder or metasurfaces as computational wave encoders [@jiang2020randomized_metamaterial; @hoang2021single_pixel_doa], whereas the present results show that a matched calibration procedure can expose direction-dependent vibration structure in an unmodified acrylic plate. Single-sensor localization has also been demonstrated with embedded microphones and contact-mounted piezoelectric sensors [@elbadawy2018lego_doa; @dipassio2023doa_single_sensor; @rutowski2024reverb_single_sensor; @rutowski2025_structural_beamforming]. Our study complements those modalities rather than displacing them. Contact and embedded sensors are attractive for integrated devices, whereas non-contact readout is especially useful when the goal is to preserve the target's native response and avoid sensor-induced loading [@ewins2000modal; @bi2013transducer_mass_loading; @nassif2005ldv_contact_sensors].

Several limitations bound the present claims. The benchmark uses a half-plane grid (0-180°) at 5° spacing, a controlled acoustic environment, a single static source, and per-object matched calibration. The LDV is a laboratory instrument rather than a field-ready sensor, and generalization to an external speech corpus has not yet been tested. Future work should test whether the observed local angle ordering can be predicted or adapted with less calibration, how performance changes under stronger damping, reverberation, multiple simultaneous sources, or moving emitters, and whether compact optical or MEMS readouts can preserve enough sensitivity for practical deployment. More broadly, passive structural complexity may prove useful for single-point wave sensing beyond this benchmark, but those broader applications remain to be demonstrated.
...
For decoding, we keep the full standardized feature space and use a grouped dictionary \(D=[d_{e,m}]\); no projection onto the reduced SVD basis is applied at inference time. The Results sections described the problem in three layers: the measured LDV response is summarized as a standardized fingerprint, calibration produces the template matrix \(H\), and readout must resolve overlap among nearby calibrated directions. Methods adds two implementation layers on top of that story. The reduced surrogate \((z, A, x)\) below is used only as an intuitive model of local overlap, whereas the implemented inference uses the grouped dictionary \(D\) and the routing machinery that follows. The formulation rests on three working assumptions: (1) over each analysis window the target behaves approximately as a linear time-invariant system, so direction-dependent responses superpose in the frequency domain; (2) the calibrated template matrix \(H\) provides a sparse prototype approximation in standardized fingerprint space; and (3) discretizing the angle ordering supplies a useful single-source surrogate over nearby directional templates. Supplementary Methods 1 gives a representative Kirchhoff-Love plate operator and Green's function derivation for assumption (1).
