...
Once speech broadens overlap locally, the next question is no longer whether the code survives. It is what kind of readout can act on that code without breaking the measured structure. Figures 4 and 5 show a narrow answer: subtraction remains scientifically admissible only when the calibrated neighborhood is preserved first [@gregor2010lista; @monga2021unrolling].

Figure 4 makes that requirement visible against the measured structure itself. In a representative 70° validation clip, broad matching initially spreads support across several nearby calibrated angles above the physically plausible band in \(H\) (Fig. 4a,b). The local gate then contracts that support around the measured neighborhood (Fig. 4c). After the first guided step, the residual drops from 1.00 to 0.48 while the fraction of update mass within 15° rises from 0.18 to 0.98 (Fig. 4d). The important change is not only that the correction becomes smaller. It becomes local in the same way as the measurement. Guided decoding therefore remains strongest under clean evaluation because it keeps subtraction inside that physically plausible neighborhood, whereas router-bypass, OMP, and dense routing progressively break the measured local order (Fig. 4e).
...
Figure 5 asks whether the same readout stays faithful to the neighborhood it uses. It does. On the shared angle frame, the calibrated template matrix \(H\) and the guided neighborhood-emphasis map show the same coarse near-diagonal organization (Fig. 5d,e), and that agreement is quantitative as well as visual: matrix-level alignment reaches \(r = 0.47\), the per-angle local-band summaries reach \(r = 0.46\), and both exceed the corresponding 95th-percentile permutation nulls (Fig. 5f).

Clean evaluation reaches the same conclusion. Guided decoding stays near the diagonal while the OMP baseline fractures into broader off-diagonal errors after committing too early, and delayed commitment remains the strongest compared rule across angle and SNR (Fig. 5a-c). The readout works here because it follows the measured neighborhood rather than replacing it with a new geometry.

If that local requirement were confined to one acrylic plate, it would remain a single-object observation. Figure 6 shows that it recurs more broadly.
...
Across five ordinary passive objects—an acrylic plate, paper cup, cardboard box, wooden board, and laptop shell—the same locally ordered directional code reappears. Each object shows structured angle-frequency fingerprints, a finite neighborhood of positive local ordering, and above-chance single-point readout under matched calibration (Fig. 6a-c). The organization first revealed by the acrylic reference object therefore recurs across a broader passive-object class.

That recurrence also clarifies what governs usability across objects. Matched object-specific calibration keeps Top-1 readout above chance across the full set (Fig. 6d), but object performance does not follow normalized overall \(|H|\) energy alone. It tracks mean top-3 overlap burden more closely. The paper cup makes the contrast explicit: it has the strongest overall energy, yet its neighboring angles overlap more broadly, so it reads out worse than cardboard. Across objects, local separability therefore matters more than raw response energy for directional readout.

The same pattern appears in frequency. A shared contrast-selection rule identifies a different informative window for each object, and the corresponding band-limited directional codes differ across those object-specific bands (Fig. 6e). What recurs is not one universal spectrum but one local directional principle expressed through different structural bands. Ordinary passive objects therefore form a class of directional sensing substrates, with directional front-end information carried by the objects themselves rather than by dedicated directional hardware alone.
...
## Discussion
Directional encoding need not be designed only into arrays and specialized sensors. Across the tested objects, passive structural vibration repeatedly produces a finite local code that one fixed vibrometric point can read after matched calibration (Fig. 6). Passive structure is therefore part of the sensing substrate, not merely a nuisance surrounding the sensor.

The same results also identify what governs recoverability. The decisive variable is not raw response energy or global template separability. It is local separability within the calibrated neighborhood. In the acrylic reference object, held-out speech preserves the code but broadens overlap among neighboring directions (Fig. 3) [@gigan2022imaging_computing_disorder; @rotter2017complex_media]. Figures 4 and 5 then show why readout fails or sharpens: subtraction breaks down when it outruns that neighborhood and succeeds when it stays inside it. The guided solver matters only as one readout that respects the measured local geometry long enough for subtraction to sharpen.
