...
Different frequency bands emphasize different directional sectors (Fig. 1e), indicating that multiple dispersive vibration pathways contribute with angle-dependent weights [@rotter2017complex_media; @kuttruff2025room_acoustics]. The sharper question is whether this code traces a readable geometry, or whether it is only a collection of different spectra.
...
The code survives. The next step is therefore no longer to prove that it exists, but to ask what kind of readout can preserve it.
...
Figure 5 asks the remaining admissibility question: does that readout stay aligned with the measured neighborhood it exploits? It does. The key point is that the learned geometry stays anchored to the measured neighborhood rather than inventing a different structure. On the shared angle frame, the calibrated template matrix \(H\) and the guided neighborhood-emphasis map show the same coarse near-diagonal organization (Fig. 5d,e). That agreement is quantitative as well as visual: matrix-level alignment reaches \(r = 0.47\), the per-angle local-band summaries reach \(r = 0.46\), and both exceed the corresponding 95th-percentile permutation nulls (Fig. 5f). Clean evaluation reaches the same conclusion: guided decoding stays near the diagonal while the OMP baseline fractures into broader off-diagonal errors after committing too early, and delayed commitment remains the strongest compared rule across angle and SNR (Fig. 5a-c). The admissible readout therefore follows the measured neighborhood it exploits.
