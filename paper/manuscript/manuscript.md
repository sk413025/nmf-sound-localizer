...
The fixed dictionary first shows a near-diagonal measured local-overlap structure around each calibrated angle (Fig. 4a). In a representative 70° validation exemplar, the initial residual excites several adjacent angles consistent with that structure, and local gating contracts that support around the correct neighborhood before the update remains localized there (Fig. 4b). Across the validation set, the fraction of update mass within 15° of the true angle rises from 0.18 before the local step to 0.98 after it (Fig. 4c).

Alternative overlap-handling rules on the fixed dictionary isolate the same constraint more directly. Router-bypass keeps staged residual updates but removes local pooling, OMP commits to a single support element while the local band is still shared, and dense routing distributes activity without preserving the measured angle order. The clean-condition comparison shows the same ordering: 0.98 for the guided solver, 0.58 for router-bypass, 0.44 for the OMP baseline, and 0.03 for dense routing (Fig. 4d). In this setting, subtraction works best when it acts on the measured local band before the solver commits to one angle or discards spatial continuity.
...
e, Per-angle clean accuracy: five-seed mean \(P(\mathrm{correct})\) across the 37 measured angles, shown as a 3-angle centered moving-average display with light \(\pm 1\) s.e.m. shading for the same four overlap-handling rules on the clean sweep. The delayed-commitment rule retains the highest clean mean accuracy overall, whereas dense routing remains near chance across almost the entire angle set.

### Matched calibration extends object-conditioned directional readout beyond the acrylic reference object (Fig. 6)
The acrylic plate is the reference object used to dissect the local-overlap problem. Across five ordinary objects, each object again yields an angle-frequency fingerprint matrix \(H\) with visible directional structure (Fig. 6a,b). The centered-magnitude curves in Fig. 6c rise rapidly across the first few modes for every object, indicating that each matched calibration again occupies a compact directional fingerprint space. Matched object-specific calibration also keeps Top-1 readout above chance across the full set (Fig. 6d), but normalized overall response energy does not by itself predict which object reads out best. Instead, the band-selectivity curves and recovered directional codes in Fig. 6e show that the informative band shifts across objects, so the extension is carried by object-conditioned structure rather than by one fixed spectral template.

Each object still requires its own calibration because response energy, informative band, and Top-1 accuracy differ across objects. Even so, the recurrence of structured \(H\) matrices, compactness, and above-chance readout across five materials shows that the acrylic plate is not the only passive structure that supports locally ordered directional readout under matched calibration. Matched calibration therefore extends object-conditioned directional readout beyond the acrylic plate while keeping the directional code tied to each object's own structural response. The recurring local organization is the same structural precondition that motivated the staged acrylic readout analysis.
...
c, Centered-magnitude SVD summary across objects. The cumulative centered-\(|H|\) energy curves rise rapidly for all five calibrated template matrices; open and square markers denote the corresponding rank-90 and rank-95 points on each curve.
d, Object-conditioned readout versus normalized overall response energy. The upper row shows per-angle Top-1 distributions with object mean \(\pm\) 95% confidence intervals, and the lower row shows normalized overall \(|H|\) energy for the same objects. Together they show that all five objects remain above chance under matched object-specific calibration, but Top-1 accuracy does not monotonically track normalized overall response energy across this sample.
e, Object-conditioned band structure across materials. The normalized band-selectivity curves and their corresponding recovered directional codes shift across objects, showing that the informative band remains object-specific rather than collapsing to one shared spectral template.
