# Ordinary passive objects can encode sound direction for single-point vibrometric readout

## Abstract
Sound direction is usually measured with arrays or specialized directional sensors.
Here we show that ordinary passive objects can themselves supply a directional code that is readable from one fixed non-contact vibrometric measurement after object-specific calibration.
In an acrylic reference object, that code appears as a compact locally ordered structure rather than as a set of unrelated angle templates.
Held-out speech preserves the structure, but broadens overlap first within nearby angle neighborhoods.
Readout therefore fails when one angle is chosen before evidence shared across that neighborhood has been consolidated.
A physics-guided solver is useful here only because it preserves the measured neighborhood before subtraction.
Across five materially different passive objects, the same organizing principle reappears, showing that passive structural complexity can be used as a directional encoding resource rather than treated only as an obstacle to direction sensing.

## Introduction
Sound direction is usually engineered by separating sensors in space. Arrays and specialized directional sensors are built so that direction is measurable at the sensor, while the vibrating or scattering object is usually treated as a nuisance. Here we ask whether that default picture is too narrow: can an ordinary passive object carry a directional code that is readable from one fixed vibrometric measurement?

Prior work makes that possibility plausible, but not yet general. Single-sensor studies have reported direction-dependent transfer functions in engineered scatterers and in structure-borne measurements [@elbadawy2018lego_doa; @dipassio2022_audio_capture_structural_sensors; @dipassio2023_waspaa_wake_word; @dipassio2023doa_single_sensor; @rutowski2024reverb_single_sensor; @rutowski2025_structural_beamforming]. Calibrated inverse formulations on a discrete angle grid provide one useful comparison class once nearby overlap must be resolved [@malioutov2005sparse_doa; @donoho2006compressed_sensing; @candes2006robust_uncertainty; @candes2008compressive_sampling; @chen2001basis_pursuit; @tibshirani1996lasso; @mallat1993matching_pursuits; @pati1993omp; @tropp2007omp]. Array-free localization is also possible with acoustic vector sensors, but those approaches still rely on specialized collocated pressure-particle-velocity instrumentation [@nehorai1994vector_sensor]. What remains open is the broader physical claim: whether ordinary passive objects themselves carry a reusable directional code, and what governs readout once nearby calibrated directions overlap.

Our hypothesis is that direction changes how sound couples into an object's vibration modes, so one fixed point records repeatable spectral reweighting across angle. Matched calibration should therefore reveal a compact local code in which nearby directions reuse shared structure instead of behaving like unrelated templates. We test that idea with a non-contact laser Doppler vibrometer (LDV), whose high-bandwidth surface-velocity readout avoids the loading introduced by attached piezoelectric patches or accelerometers [@rothberg2017ldv; @castellini2006ldv; @wagner2021_laser_microphone_calibration; @ewins2000modal; @bi2013transducer_mass_loading; @nassif2005ldv_contact_sensors].

The acrylic plate is the clearest reference object for exposing that code, but not the paper's endpoint. The broader question is whether the same organizing principle recurs across ordinary passive objects, and whether recoverability is governed by local separability rather than by response energy alone. More broadly, the study sits alongside wave systems that treat complex scattering as an encoding resource rather than as noise [@fink1997time; @duarte2008single; @gigan2022imaging_computing_disorder; @rotter2017complex_media; @jiang2020randomized_metamaterial]. The central claim is therefore broader than one reference object: ordinary passive structures can carry direction, and local separability determines whether that code stays readable.
...
An empirical component bridge further shows that angle mainly redistributes weight across a small set of reusable centered-magnitude spectral patterns (Fig. 1b).
...
In the full angle-frequency matrix, neighboring directions reuse related structure. Reconstruction error drops sharply once a small retained basis is kept, the inter-angle similarity map is strongest near the diagonal, and the compact 2D embedding shows the same angle-ordered geometry in a lower-dimensional view (Fig. 2d-f).
For the centered-magnitude view used in Fig. 2,

$$
H_{\mathrm{fig}}[k,e] = |H[k,e]| - \frac{1}{E}\sum_{e'=1}^{E}|H[k,e']|
$$

the spectrum decays rapidly (Fig. 2a-c): the first six components capture 80.3% of the energy and eight capture 85.1%. The measured fingerprints therefore occupy a compact angle-ordered space with strong continuity across nearby directions.

That compactness matters because it is exactly what we expect if changing direction mainly reweights a limited set of structural modes instead of generating fully independent fingerprints at every angle.
Under small-amplitude dynamics, the single-point velocity response can be written as a superposition of dispersive structural modes, each with its own spectral pattern \(s_m(\omega)\) and direction-dependent coupling \(\alpha_m(\theta)\):

$$Y(\omega;\theta) \approx \sum_{m} s_m(\omega)\,\alpha_m(\theta), \qquad (1)$$

where \(Y(\omega;\theta)\) is the complex frequency response at the LDV measurement point for incidence direction \(\theta\), and the sum runs over a limited set of contributing modes (Supplementary Methods 1) [@ewins2000modal; @meirovitch2001fundamentals]. In this view, changing direction mainly reweights a limited set of dispersive responses, so neighboring angles share structure without becoming identical. The centered-magnitude components should therefore be read as empirical summaries of that shared structure, not as resolved physical eigenmodes. What matched calibration reveals is a locally ordered directional code: nearby angles remain related, but not interchangeable.

That observation changes the form of the decoding problem. Once the measured fingerprints are placed on a discrete angle grid, direction recovery is no longer a search over 37 unrelated references. It becomes a local separability problem: neighboring templates can become jointly plausible before a final angle is chosen. Held-out speech next tests whether that local code survives a realistic source change.

![](../figures/fig02_svd-physical-dictionary.jpg)

**Fig. 2 | Matched calibration reveals a compact local directional code.**
a, Singular-value spectrum of the centered-magnitude fingerprint matrix. Energy accumulates rapidly across the 37-angle grid: six components capture 80.3% of the energy and eight capture 85.1%, showing that most measured directional structure sits in a small component set. The same early components also carry most of the angle ordering.
...
e, All-angle reconstruction fidelity under rank-\(r\) truncation. Per-angle centered-magnitude RMSE drops sharply by the same six-component regime highlighted in panel a, showing that the local code is captured before many additional components are added.
...
Held-out speech asks whether the locally ordered code revealed by calibration survives a realistic source change. It does, but not in the same form as white-noise calibration. White noise samples the calibration map densely in each short window. Speech samples the same map unevenly because its energy is sparse and non-stationary in time-frequency. The key consequence is local, not global: nearby angles become harder to separate before directional structure disappears.
...
f, Stage-0 correlation-based first-choice diagnostic versus SNR. The white-noise curve (blue, clip-level standard error of the mean (SEM) shading) is recomputed on synthetic noisy white-noise datasets. The speech-plus-babble curve (orange, 5-seed mean \(\pm\) standard error of the mean (SEM) shading) comes from a separate five-seed sweep. Both curves decline monotonically as noise increases, so noise enlarges the same local-overlap problem.

### Readout succeeds only when the measured neighborhood is preserved (Figs. 4 and 5)
Figures 4 and 5 answer one bounded question. Once local overlap is the governing difficulty, what kind of readout remains scientifically admissible? The answer is narrow: subtraction helps only after the measured neighborhood has been preserved [@gregor2010lista; @monga2021unrolling].

Figure 4 shows that requirement directly against measured structure. In a representative 70° validation clip, broad matching initially excites several nearby calibrated angles above the physically plausible band in \(H\) (Fig. 4a,b). The local gate then contracts support around the correct neighborhood and keeps the update confined there (Fig. 4c). After the first guided step, the residual drops from 1.00 to 0.48, and the fraction of update mass within 15° rises from 0.18 to 0.98 (Fig. 4d). The update becomes both smaller and more local.

The clean decoder comparison isolates the same requirement in simpler form. Router-bypass keeps residual correction but removes local pooling. The OMP baseline commits to one support element while the local band is still shared, and dense routing spreads activity without preserving measured angle order. The clean means follow the same ranking: 0.98 for the guided solver, 0.58 for router-bypass, 0.44 for the OMP baseline, and 0.03 for dense routing (Fig. 4e). Once overlap is local, early commitment is the wrong operation.
...
d, Residual purification. Left, relative residual norm across the guided steps for the representative clip. Right, validation-wide cumulative update mass within radius before and after one local step across 1,924 clips; the mass within 15° rises from 0.18 to 0.98, so almost all correction moves into the physically plausible neighborhood before subtraction proceeds.
e, Clean-condition Top-1 comparison of four overlap-handling rules on the same fixed dictionary (individual seeds shown as dots; large markers and error bars indicate mean \(\pm\) s.e.m.). The displayed clean means are 0.98 for the guided solver, 0.58 for router-bypass, 0.44 for the OMP baseline, and 0.03 for dense routing. Accurate subtraction appears only when the local angle band is preserved first.

Figure 5 asks a narrower question: does that readout stay aligned with the measured neighborhood it exploits? It does. The calibrated template matrix \(H\) and the guided neighborhood-emphasis map share the same coarse near-diagonal organization on the same angle frame (Fig. 5d,e). Matrix-level agreement reaches \(r = 0.47\), and the per-angle local-band summaries reach \(r = 0.46\) with a mean absolute gap of 0.23 (Fig. 5f). The readout therefore follows the measured neighborhood at the scale that matters for decoding.

The same alignment explains the prediction behavior. Under clean evaluation, guided decoding stays near the diagonal, whereas the OMP baseline fractures into broader off-diagonal errors after committing too early (Fig. 5c). Across angle, delayed commitment performs best across most of the 37 measured directions, and across SNR it degrades least as added noise increases (Fig. 5a,b). The admissibility question is therefore closed. The remaining question is whether the same code recurs beyond the acrylic reference object.
...
b, Per-angle clean accuracy across the 37 measured angles. After light 3-angle smoothing, the delayed-commitment rule remains highest across most angles, whereas dense routing stays near chance across much of the grid; light shading shows \(\pm 1\) s.e.m. around the five-seed mean \(P(\mathrm{correct})\).
...
f, Quantitative structure alignment. Top, normalized local-band summaries from the measured and learned maps across angle. Bottom, concordance scatter of the same per-angle scores. The full-matrix agreement reaches \(r = 0.47\), and the per-angle summaries reach \(r = 0.46\) (mean absolute gap \(= 0.23\)). The readout therefore stays tied to the measured neighborhood rather than drifting to an unrelated learned geometry.

### The same locally ordered directional code recurs across ordinary passive objects (Fig. 6)
The acrylic result is not a special case. Across five ordinary passive objects, matched calibration again reveals a finite locally ordered directional neighborhood that supports above-chance readout. Correlation-decay width, effective rank, and informative band differ across materials, but the same directional code remains visible in every case (Fig. 6a-c).
...
The same logic carries into frequency. A shared contrast-selection rule identifies a different informative window for each object, and the corresponding band-limited directional codes differ across those object-specific bands (Fig. 6e). What recurs across the tested set is not one universal spectrum. It is one directional principle expressed through object-specific informative bands.

![](../figures/fig06_universality.jpg)

**Fig. 6 | A common directional code recurs across ordinary passive objects.**
...
## Discussion
Direction sensing is usually engineered by arranging sensors in space. The central result here is that ordinary passive structure can supply that directional code on its own. Across the tested objects, one fixed vibrometric measurement becomes informative after matched calibration because structural vibration organizes direction into a finite locally ordered neighborhood rather than into unrelated angle labels. Passive structure is therefore not only something a directional sensor interrogates. It can itself instantiate the directional encoding.
