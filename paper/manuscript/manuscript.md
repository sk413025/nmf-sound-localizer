...
Noise worsens the same failure mode. Greedy accuracy declines monotonically as additive noise increases in both the white-noise and speech conditions (Fig. 3f). Figure 3 therefore sets the hinge for the remainder of the paper: speech does not remove directional information from measured fingerprint space, but it broadens overlap among neighboring calibrated templates. The remaining readout requirement is therefore specific: pool evidence within that local neighborhood before committing to one reported direction.
...
### Nearby overlap narrows after one refinement step (Fig. 4)
Figure 4 turns that readout requirement into one concrete 70° case. The initial match is broad across nearby calibrated directions, consistent with the local ambiguity seen in Fig. 3e, but one refinement step concentrates that support around the correct neighborhood and leaves less unexplained signal [@gregor2010lista; @monga2021unrolling]. The compact callouts summarize the same shift numerically: the fraction of the match that falls within 15° of the true direction rises from 0.18 to 0.98, while the remaining residual falls from 1.00 to 0.48.

The panels trace the same overlap-to-readout sequence directly: support first spreads across a local neighborhood, then sharpens, and the residual falls once nearby evidence has been consolidated. Figure 4 therefore links the measured local overlap diagnosed in Fig. 3 to one benchmark behavior, namely preserving the local ordering of the calibrated fingerprints before forcing an isolated choice. The clean-condition comparison in Fig. 4d is consistent with that interpretation: in this restricted setting, the strongest performance comes from the comparison that keeps the response concentrated near the correct neighborhood.

![](../figures/fig04_solver-dynamics.jpg)

**Fig. 4 | One refinement step resolves nearby overlap and reduces the residual.**
...
We next ask whether that same principle holds across the full benchmark. If local overlap in the measured template matrix \(H\) is the dominant constraint, then the strongest comparison should be the one that remains aligned with that measured angle ordering and whose predictions stay concentrated near the correct direction once nearby evidence is pooled. That is the pattern observed in Fig. 5: the neighborhood-emphasis map follows the same local band seen in \(H\) (Fig. 5c), and the benchmark predictions stay most concentrated near the correct neighborhood in both the row-normalized comparison block and the representative angle-specific profiles (Fig. 5b,d).

Across noise levels and all 37 angles, the most locally concentrated comparison remains the most accurate within this benchmark, whereas immediate or weakly constrained alternatives degrade more sharply (Fig. 5a,e). The measured ordering therefore still favors local evidence pooling before final commitment once nearby template overlap broadens.
...
b, Row-normalized comparison block across the four benchmarked approaches. The most locally concentrated comparison stays nearest the diagonal, whereas the less locally constrained alternatives show broader off-axis leakage or collapse toward a preferred output mode.
...
## Discussion
The main advance here is that matched calibration turns the acrylic plate's direction-dependent response into a calibrated single-point sensing pathway that remains readable under source change. At one fixed LDV position, the plate produces repeatable measured fingerprints, those fingerprints organize into a compact template matrix \(H\), and held-out speech preserves their local angle ordering even as it broadens overlap among neighboring calibrated directions [@gigan2022imaging_computing_disorder; @rotter2017complex_media]. The bounded conclusion is therefore clear: in this acrylic-plate benchmark, sound direction can be read from one calibrated vibrometric measurement when the readout pools local evidence before committing to a single calibrated direction.

The five-object screen serves a narrower role. Under matched object-specific calibration, each object again yields structured fingerprints and early energy capture in the centered-magnitude SVD view, suggesting that the object-response -> fingerprint -> \(H\) pathway can recur beyond one plate. At the same time, normalized response energy, informative bands, and Top-1 performance differ across objects, so this extension remains exploratory and does not yet establish a shared readout law across materials.
