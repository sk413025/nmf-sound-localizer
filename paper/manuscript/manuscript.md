...
A hybrid schematic-plus-data view then shows the governing principle: direction does not create unrelated templates, but mainly reweights a small set of shared structural responses whose mixture reproduces representative single-point fingerprints (Fig. 1b).
...
b, Hybrid governing-principle view. A compact schematic traces how incoming angle excites the passive plate and is read out at one fixed LDV spot. The adjoining decomposition shows three shared centered-magnitude responses, their angle-specific weights at representative angles (0°, 90°, 180°), and the resulting measured versus three-response reconstructed fingerprints. Direction therefore reweights shared structural responses rather than creating unrelated templates.
...
Matched calibration shows that these fingerprints are not a set of unrelated angle templates. In the full angle-frequency matrix, neighboring directions reuse related structure (Fig. 2d-f), and the same early low-rank regime already captures most of the centered-magnitude energy (Fig. 2a-c): six components account for 80.3% and eight account for 85.1%.
