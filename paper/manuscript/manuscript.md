...
c, Neighborhood-coherence map in split-triangle form: lower-left = calibration view of the local neighborhood, upper-right = speech-side view of the same local neighborhood. Speech preserves the coarse angle ordering and near-diagonal structure while widening the local band.
...
b, Per-object template matrices \(H\). Shared-normalization heatmaps show structured angle-frequency encoding across the five objects; the colored horizontal band on each heatmap marks the object-specific informative band.
