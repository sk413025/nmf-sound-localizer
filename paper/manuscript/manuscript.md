...
Within this benchmark, the OMP baseline makes an immediate single-choice match on the calibrated dictionary, the guided solver uses the same residual-correction scaffold with learned neighborhood-aware local gating, router-bypass keeps that scaffold without the learned gate, and dense routing spreads evidence broadly across angles rather than concentrating it within one local neighborhood.
...
Across noise levels and all 37 angles, the guided solver remains the most accurate within this benchmark, whereas the immediate or weakly localized alternatives degrade more sharply (Fig. 5a,e). These benchmark results therefore support local evidence pooling before final commitment once nearby template overlap broadens.
...
Across a small five-object screen, matched calibration again reveals structured directional encoding beyond the acrylic plate. Each object yields its own angle-frequency fingerprint pattern in the calibrated template matrix \(H\) (Fig. 6a,b), and all five objects remain above chance under the same matched-calibration screening procedure despite clear differences in performance across objects (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e), indicating that the directional code recurs through object-specific response structure rather than through one fixed spectral pattern. The centered-magnitude SVD summary in Fig. 6c provides a supporting compactness comparison across the same screen. The extension is therefore bounded: matched calibration can reveal directional structure beyond one plate, but this screen does not establish how broadly that structure persists across materials.

The same screen also limits that extension. Normalized response energy does not monotonically track Top-1 accuracy across the five objects (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e). This five-object screen therefore supports recurrence under matched calibration, not transfer across objects or a validated material law.
...
The five-object screen shows bounded recurrence rather than broad material invariance. Under matched object-specific calibration, each object yields structured fingerprints, above-chance screening performance, and a different informative frequency band, indicating that usable directional encoding can extend beyond one plate within this small screen. At the same time, normalized response energy, informative bands, and Top-1 performance differ across objects, so this extension remains exploratory and does not yet show a common readout pattern across materials. The compactness summary in Fig. 6c is consistent with that interpretation, while the cross-object variation in screening and band placement keeps the conclusion bounded.
