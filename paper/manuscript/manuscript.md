...
Figure 3 identifies the design criterion: speech broadens overlap along the physical angle manifold, so nearby evidence must be integrated before a final directional commitment. In the guided solver (Fig. 4a; Methods: Inference algorithms and network architecture) [@gregor2010lista; @monga2021unrolling], the physical match is first concentrated into one local neighborhood, and only then is the residual recomputed on the cleaner landscape left behind. Panel a reduces that mechanism to a thin broad-match -> local-concentration -> cleaner-residual strip rather than a solver diagram.

Figure 4 visualizes that physical response on a shared 70° validation exemplar. Panel b now places the broad physical match \(g_t(\theta)\), the local gate \(w_t(\theta)\), and the localized update \(\Delta x_t(\theta)\) on one shared axis so the broad-to-local transition can be read at a glance. Panel c then shows the consequence of that local step on the same scale: the next correlation profile \(g_{t+1}(\theta)\) is suppressed around the solved neighborhood, while compact callouts report the corresponding inward mass shift and first residual drop.

The clean-condition decoder comparison in Fig. 4d shows the practical payoff: among the four decoder families defined above, the guided solver outperforms router-bypass, the OMP baseline, and dense routing. Figure 5 asks whether that same advantage persists across the full held-out benchmark and noise sweeps.
...
### Guided solver follows the measured angle manifold and resists noise (Fig. 5)
...
The mechanistic anchor is panel c. The dictionary \(H\) has a banded correlation structure because nearby angles share related fingerprints, and the guided neighborhood map follows that same smooth manifold while sharpening its local diagonal concentration (Fig. 5c). The physical manifold therefore remains the primary structure, with the guided response following it rather than replacing it with an unconstrained classifier. The confusion-family block shows the same ranking geometrically: the guided solver stays close to the diagonal, the OMP baseline spills off axis, router-bypass leaks locally, and dense routing collapses toward a preferred output mode (Fig. 5b).
...
c, Correlation structure of the measured physical dictionary \(H\) (top) and the guided neighborhood map (bottom), showing that guided routing follows the physical angle manifold.
...
### Direction-dependent encoding recurs in an exploratory five-object screen (Fig. 6)
