...
These fingerprints are specific and repeatable across trials (Fig. 1d; Methods: Fingerprint similarity analysis). On a {TBD_N_ANGLES}-angle white-noise evaluation set ({TBD_WN_EVAL_CLIPS_PER_ANGLE} clips per angle), within-angle similarity is consistently high while between-angle similarity is markedly lower, indicating that the fingerprints are deterministic and angle-specific rather than random noise artifacts (Fig. 1d; quantitative discriminability analysis in Fig. 4).
...
**Attention routing equations (Fig. 3).** In the rank-\(r\) projected space, at stage \(t\) we start from residual \(r_t\in\mathbb{R}^r\) and compute correlations \(g_t=A^\top r_t\in\mathbb{R}^E\). We parameterize routing weights over atoms via dot-product attention [@vaswani2017attention]. A query is computed from the current state,
$$
