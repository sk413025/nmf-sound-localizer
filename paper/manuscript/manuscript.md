...
Figure 3 sets that readout requirement in operational form: once speech broadens overlap among neighboring templates, hard one-angle selection discards the measured local continuity too early. In other words, the failure of classical greedy pursuit here is not just that it is greedy, but that it must isolate one support element while the measured response still spans a coupled local neighborhood. We therefore use a physics-guided solver that writes that local continuity into each residual-correction step by scoring the residual against the calibrated dictionary, pooling that match within a local angle neighborhood, and only then subtracting [@gregor2010lista; @monga2021unrolling].
...
Panel d is a restricted clean-condition check on the same point. Accuracy is highest for the guided solver (0.98), lower for router-bypass (0.58), lower again for the OMP baseline (0.44), and near chance for dense routing (0.03) (Fig. 4d). Within this restricted check, accuracy peaks only when local evidence is preserved before subtraction.
