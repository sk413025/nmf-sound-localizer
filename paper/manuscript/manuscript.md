...
## Abstract
Passive objects can encode sound direction in their vibration patterns, but it is unclear whether one fixed vibrometric measurement can recover that direction after calibration. Here, matched calibration on a 37-angle half-plane (0-180° in 5° steps) shows that a passive acrylic plate produces repeatable directional fingerprints in a single non-contact laser Doppler vibrometer (LDV) readout. Held-out speech preserves that directional structure but broadens overlap among nearby angles. The main readout failure therefore appears when several neighboring directions remain plausible and the decoder commits too early to one of them. A physics-guided solver shows that narrowing this local ambiguity before subtraction stabilizes clean-condition readout. A bounded five-object screen further suggests that the same calibrated sensing pathway can recur beyond the acrylic plate. These results establish single-point vibrometric direction readout in this benchmark and identify nearby-angle overlap as the main operational limit.

## Introduction
Passive objects can encode sound direction in how they vibrate under directional forcing. The question here is whether one fixed vibrometric measurement, taken at one point on the object, can recover that direction after object-specific calibration. In this framing, the object itself becomes the sensing element, and the main readout challenge is whether nearby directions remain distinct enough to be separated from one another.
...
That hypothesis makes two concrete predictions. First, calibration should reveal a compact measured space in which neighboring directions remain locally ordered. Second, speech should challenge readout mainly by broadening overlap among nearby calibrated directions. Stable decoding would then depend on resolving that local ambiguity before a final angle is committed.
...
### Speech preserves direction but destabilizes direct one-angle matching (Fig. 3)
Held-out speech tests whether the compact local structure identified in Figs. 1 and 2 survives a change in source content. White noise is the calibration probe used to estimate the template matrix \(H\), so it exposes the direction-dependent vibration response with minimal source structure. Speech poses the harder test because structured spectrotemporal variation can broaden overlap among neighboring calibrated fingerprints while leaving the underlying directional code intact.

That directional code survives. Under white-noise excitation, within-angle Pearson correlation is near-perfect and clearly separated from between-angle correlation (\(\bar{r} = 1.000\) versus 0.724; \(d = 2.83\); Fig. 3a). Under speech, within-angle similarity drops to 0.907, but it remains higher than between-angle similarity (0.798), with \(d = 1.95\) (Fig. 3b). The per-angle discriminability margin stays positive at every angle, although its mean decreases from 0.28 under white noise to 0.11 under speech (Fig. 3c). Speech therefore weakens local separability without erasing the directional code.

The failure appears at the first discrete commitment. The correlation-based first-choice diagnostic isolates that initial greedy decision before any residual correction. It succeeds on most white-noise fingerprints, where the pairwise similarity map is close to identity, but it collapses on held-out speech, where similarity spreads across neighboring calibrated angles while remaining locally ordered (Fig. 3d,e). Once several adjacent templates become jointly plausible, immediate one-angle commitment becomes unstable even though directional structure remains present.

Added noise amplifies the same instability. The correlation-based first-choice diagnostic declines monotonically as additive noise increases in both stimulus conditions (Fig. 3f). The operational bottleneck is therefore not whether speech still contains directional information, but whether nearby directions can be disambiguated before subtraction begins.
...
f, Dose-response curves for the same correlation-based first-choice diagnostic versus SNR for white-noise signal (blue, clip-level standard error of the mean (SEM) shading) and speech signal with babble noise (orange, 5-seed mean \(\pm\) standard error of the mean (SEM) shading), both declining monotonically with increasing noise.

### Resolving nearby-angle ambiguity before subtraction stabilizes decoding (Fig. 4)
Figure 3 isolates the bottleneck: several nearby angles remain plausible before subtraction begins. The next question is whether narrowing that neighborhood first yields a cleaner update. To test that point, we use a physics-guided solver that scores the residual against the calibrated dictionary, concentrates the match within a local angle neighborhood, and only then subtracts [@gregor2010lista; @monga2021unrolling].
...
Figure 5 asks a narrower question: once local ambiguity is handled before subtraction, what prediction structure remains around the calibrated neighborhood? Panel c suggests the answer is still local. The calibrated template matrix \(H\) shows a near-diagonal band of local overlap, and the guided solver's neighborhood-emphasis map retains a coarse near-diagonal band (Fig. 5c).
...
Across a small five-object screen, the matched-calibration sensing pathway is not unique to the acrylic plate. Each object yields its own structured angle-frequency fingerprint pattern in the calibrated template matrix \(H\) (Fig. 6a,b), and all five objects remain above chance under matched object-specific calibration despite clear differences in screening performance (Fig. 6d). The per-object frequency summaries further show that the informative band can shift across objects (Fig. 6e), consistent with recurrence through object-specific response structure rather than through one fixed spectral pattern. The centered-magnitude SVD summary in Fig. 6c provides a supporting compactness comparison across the same screen.

That broader consequence remains bounded. Within this five-object screen, matched calibration can be re-established across different objects, but normalized response energy does not monotonically track Top-1 accuracy (Fig. 6d), and the informative band does not stay fixed across objects (Fig. 6e). The result therefore supports recurrence of the sensing pathway under object-specific calibration, not transfer across objects or a material-agnostic law.
