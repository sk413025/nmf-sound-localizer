...
Because the structure is itself the encoder, the readout mechanism must not significantly perturb the object’s dynamics. We therefore performed a sensor-loading control by attaching a small contact mass (or contact transducer) at the measurement location to mimic the local stiffness/damping changes induced by contact sensing [@placeholder_sensor_loading_review]. Relative to non-contact LDV, loading reduces fingerprint separability (within-angle similarity `[X]±[Y]` to `[X]±[Y]`, between-angle similarity `[X]±[Y]` to `[X]±[Y]`) and degrades DOA accuracy from `[X]%` to `[Y]%` at SNR = `[X] dB` (n=`[X]` trials). This supports the use of non-contact LDV as an enabling measurement modality for preserving and interrogating physical DOA encoding.
...
We measured out-of-plane structural vibration using a non-contact laser Doppler vibrometer (LDV; `[manufacturer/model]`), targeting a single fixed point \((x_L,y_L)\) on each object. Non-contact LDV readout avoids sensor loading that can perturb the encoder itself: attaching a contact sensor can change local mass and damping and modify boundary conditions, potentially shifting or smearing the dispersive signatures used for encoding [@placeholder_ldv_review; @placeholder_sensor_loading_review]. To mitigate speckle and surface reflectivity issues, we used `[reflective tape / surface preparation]` and verified stable LDV signal quality before each run.
...
**Sensor-loading control.** To test whether contact sensing perturbs the physical encoding, we repeat a subset of trials while attaching a contact mass (or contact transducer) at \((x_L,y_L)\). The added loading is `[X] g` (or `[X]` sensor type) and is applied using `[X]` adhesive. We then recompute the fingerprint similarity metrics (Fig. 1), the singular spectrum of \(H\) (Fig. 2), and DOA accuracy/RMSE (Figs. 4–6) under identical stimulus and angle conditions.
