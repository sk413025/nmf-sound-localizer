...
From this view, three questions follow: does incidence direction leave a reproducible spectral signature, is that signature governed by a limited number of dominant channels, and can a solver that respects that geometry decode direction more reliably when content variation broadens the fingerprints? Guided by this logic, we first establish the encoding itself, then its low-rank physical structure, then the need for and behavior of a geometry-aware solver, and finally how far that same picture recurs across a small object-specific screening set (Figs. 1-6).
...
Because the fingerprints vary smoothly rather than arbitrarily, they admit a calibration-based sparse surrogate. For exposition one may project the standardized fingerprint and calibration dictionary as \(z=U_r^\top \tilde{y}\) and \(A=U_r^\top H\). In that reduced feature space, direction-of-arrival (DOA) inference becomes a structured sparse inverse problem:

$$z \approx A\,x, \quad \text{subject to} \quad \lVert x \rVert_0 \le K, \qquad (2)$$

where \(z\) is the reduced-order test fingerprint, \(A\) is the reduced-order calibration dictionary, \(x\) is a sparse coefficient vector whose dominant support identifies the source direction, and \(K\) is the pursuit depth, that is, a small residual-correction budget for local overlap and noise rather than an assumption of multiple simultaneous sources (Methods: Inference algorithms and network architecture). We use Eq. 2 only as an interpretable reduced-order surrogate; the executed solver remains in the full standardized feature space. In practical terms, Fig. 2 sets the design criterion for the next sections: the decoder should exploit a smooth neighborhood structure, not treat the 37 directions as unrelated classes.
...
d, Stacked angle-resolved correlation-based greedy diagnostic traces for white noise and speech, shown with light clip-level uncertainty bands: the classical greedy baseline performs strongly on most white-noise fingerprints but drops to near chance across the calibrated grid for held-out speech clips.
...
Figure 3 identifies the design criterion: keep the dictionary-grounded physical score, but delay the hard decision until nearby angles have been compared jointly. We implement that idea with a physics-guided routed pursuit (Fig. 4a; Methods: Inference algorithms and network architecture) [@gregor2010lista; @monga2021unrolling]. Each stage starts from the same physical correlation used by the classical baseline, then uses a learned local router to reweight neighboring hypotheses before residual subtraction. The goal is not to leave the physical model behind, but to resolve the local ambiguity that speech introduces along the angle manifold.

Figure 4 visualizes that mechanism on a shared 70° validation exemplar. The raw correlation \(g_t(\theta)\) is broad across neighboring directions (Fig. 4b), but the routed weight \(w_t(\theta)\) concentrates within that same neighborhood rather than inventing distant alternatives. Panel d confirms that these angle-axis curves are reductions of the model's mode-resolved routing and update tensors, so the sharpening seen in panels b and c is tied to the same internal quantities shown schematically in panel a.

After routing, the update \(\Delta x_t(\theta)\) becomes markedly more localized, and residual correction changes the next correlation landscape in a dictionary-consistent way (Fig. 4c). The clean-condition ablation in Fig. 4e shows the practical payoff: among the four decoder families used throughout the paper, the guided solver outperforms router-bypass, the OMP baseline, and dense routing. Figure 5 asks whether that same advantage persists across the full held-out benchmark and noise sweeps.
...
### Direction-dependent encoding and low-rank continuity recur across structurally distinct everyday objects (Fig. 6)
Figure 5 established that manifold-aligned routing improves decoding on the acrylic benchmark. Figure 6 asks a different question: does the encoding principle itself recur across structurally different objects when each object is given its own calibration? Across the cardboard box, wooden board, acrylic plate, paper cup, and laptop shell, object-specific calibration still produced structured angle-frequency transfer functions and above-chance screening performance (Fig. 6a,b). The principle is therefore not confined to one favorable surface, even though the present evidence remains object-specific rather than cross-object.

The same row-wise mean-centered magnitude SVD used in Fig. 2 showed that a modest number of modes captured most centered energy for every object (Fig. 6c), indicating that low-rank continuity also recurs across the screened set. Figure 6d then asks whether simple overall response strength explains screening performance. It does not: normalized response energy does not rank the objects in the same order as Top-1 accuracy, and the angle-level MAE matrix does not reveal detectable cross-material heterogeneity in this committed sample (Friedman chi-square(4) = 1.427136, \(p = 0.839465\)).
...
where \(V[k,t]\) is the complex STFT coefficient at frequency bin \(k\) and frame \(t\), and \(T\) is the number of retained frames. The spectrum is restricted to [300, 3,000] Hz (\(F = 346\) bins). Because the observed fingerprints are magnitude statistics, each clip is summarized by a log-power feature vector \(y[k]=\log_{10}(\widehat{S}+\epsilon)\), then standardized per frequency bin using white-noise calibration statistics to yield a normalized feature \(\tilde{y}[k]\) (Supplementary Methods 2). These normalized log-power fingerprints are the observables used for downstream inference. Accordingly, the sparse model in Results (Eq. 2) should be read as a feature-space approximation derived from the physical response in Eq. 1: the complex response \(Y(\omega;\theta)\) is converted into magnitude statistics, discretized over frequency and angle, and standardized before sparse matching. Equation 2 is therefore not a literal complex-domain superposition law and not a direct power-additivity statement.

### Physical dictionary formulation and centered-magnitude SVD
The normalized features serve as the basis for constructing the angle-indexed dictionary used in inference. Per-angle prototypes are averaged across calibration trials to form columns \(h_e\), where \(e\) indexes one of the \(E\) candidate directions, yielding the standardized fingerprint dictionary \(H=[h_1,\dots,h_{37}]\in\mathbb{R}^{F\times E}\), with \(E=37\). Figure 2 does not decompose the ideal complex response \(\mathcal H\) directly. Instead, matching the committed Fig. 2 generator, it analyzes the centered-magnitude matrix
...
For exposition, one may also form a reduced-order surrogate by projecting the standardized fingerprint and dictionary into a retained singular subspace as \(z=U_r^\top \tilde{y}\) and \(A=U_r^\top H\). This projection is used only to motivate Eq. 2. The executed runtime path reported here does not apply PCA/SVD before decoding and instead retains the full \(F=346\) standardized feature space.

### Inference algorithms and network architecture
Using the sparse inverse viewpoint introduced in Results, DOA inference seeks a small set of angle-indexed calibration components that explain the observed standardized fingerprint. Equation 2 is the reduced-order surrogate for that viewpoint. The executed model instead uses the full standardized feature space with a grouped dictionary \(D=[d_{e,m}]\) and no PCA/SVD projection. The formulation rests on three working assumptions: (1) over each analysis window the target behaves as a linear time-invariant system, so direction-dependent responses superpose in the frequency domain; (2) \(Hx\) is a sparse prototype approximation in standardized fingerprint space, not a literal power-additivity law; and (3) discretizing the angle manifold provides a useful single-source surrogate over nearby directional templates. The representative Kirchhoff-Love plate operator and Green's function derivation supporting assumption (1) are given in Supplementary Methods 1.
...
where \(r_t\) is the current residual and each entry of \(g_t\) quantifies how well a candidate direction explains the unexplained observation through the physical dictionary. A transformer encoder (embedding dimension \(d_\mathrm{model}=128\), 2 attention heads, 1 encoder layer in the cited primary guided-solver run) uses the current residual to query the dictionary and produces expert-level routing scores from scaled dot-product interactions:

$$s_t[e] = \frac{\langle q_t, k_e \rangle}{\sqrt{d_k}}, \qquad (5)$$

where \(q_t\) is a learned query derived from the current residual, \(k_e\) is the learned key associated with direction group \(e\), and \(d_k\) is the key dimension. In this solver family, routed selection is implemented with Gumbel-family gates. The `use_hard_gumbel` switch is optional in the codebase; in the cited primary guided-solver run it is enabled. The forward routing step therefore behaves as a manifold-aware discrete gate over neighboring angle hypotheses rather than as a purely soft attention weight. These routed weights gate the physical match scores to produce a sparse update:
...
where \(\mathcal{L}_\mathrm{rec} = \lVert r_K \rVert_2^2\), \(\mathcal{L}_\mathrm{mono}\) encourages stagewise residual descent, and \(\mathcal{L}_\mathrm{cls}\) is the cross-entropy loss over the expert-level readout \(\bar s[e]\). Equation 9 gives the main loss; in the cited primary run, the implementation also adds an auxiliary early-epoch teacher-warmup cross-entropy term for the first 10 epochs. The loss weights in the executed main objective are \((\alpha,\beta,\gamma)=(1.0,\,0.2,\,0.5)\). Optimization is performed with Adam (learning rate \(10^{-3}\), weight decay \(10^{-4}\)) for 20 epochs with batch size 32.
...
### Solver ablations
The manuscript uses four stable decoder-family labels in Figs. 4 and 5: *guided solver*, *router-bypass*, *OMP baseline*, and *dense routing*. In the upstream clean-condition sweep, *No Transformer* maps to *router-bypass*, *No Type Bias* is the active *guided solver* family, and *Fixed Heuristic*, *G-Fixed*, and *G-Teacher* are numerically identical fixed-routing OMP-family variants collapsed into the paper-facing *OMP baseline*. That OMP baseline is distinct from the correlation-based greedy diagnostic in Fig. 3, which is reported separately to isolate the failure of immediate single-choice matching under content variation. *Dense Routing* remains the uniform-weight failure mode.
