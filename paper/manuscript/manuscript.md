...
Matched object-specific calibration keeps Top-1 readout above chance across the full set (Fig. 6d, top). The lower row does not establish a fitted law, but it does place object mean Top-1 directly against correlation-decay width while encoding normalized overall \(|H|\) energy in marker area. In that descriptive comparison, response energy alone does not order the five objects: the paper cup carries the strongest normalized overall energy but still sits at broader overlap and lower readout than cardboard. The same shared contrast-selection rule also picks different informative frequency windows across objects, and the band-limited directional codes recovered from those windows differ across objects as well (Fig. 6e). The informative band therefore remains object-conditioned rather than collapsing to one shared spectral template.

Each object therefore still requires its own calibration because overlap width, compactness, and informative band remain object-specific. Even so, the recurrence of a finite locally ordered neighborhood, structured \(H\) matrices, and above-chance readout across five materials shows that the acrylic plate is not a singular exception. What recurs across materials is not one universal spectrum, but an object-conditioned local ordering that the decoder can exploit under matched calibration.
...
The five-object comparison extends that observation across passive objects. After each tested object is calibrated on its own, structured directional fingerprints and above-chance readout appear in cardboard, wood, paper-cup, and laptop-shell objects as well. Normalized response energy, informative bands, and Top-1 performance remain object-specific, but the recurrence of compact, locally organized directional structure across these materials shows that the staged acrylic readout analysis exposed a broader structural constraint rather than a one-object curiosity. What recurs across objects is a finite locally ordered organization of the calibrated fingerprints, which is the same structural precondition that motivated the staged acrylic readout analysis.
