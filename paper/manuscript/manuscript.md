...
f, Family-to-measured neighborhood alignment. Bars show a local-band agreement score for each decoder family, defined by retained within-15° local mass with a secondary weighting from the agreement between the family’s anglewise local-band profile and the measured neighborhood profile. Open circles show the corresponding full-matrix correlation to the measured neighborhood as a secondary reference. Families that keep support both local and correctly ordered score highest on the primary local-band metric.
...
Top-1 accuracy is reported on the discrete angle grid. Angular error is the minimal angular difference \(\Delta(\hat\theta,\theta)=\min_{k\in\mathbb{Z}}|\hat\theta-\theta+360k|\), and root-mean-square error (RMSE) = \(\sqrt{\frac{1}{N}\sum_i \Delta(\hat\theta_i,\theta_i)^2}\). Unless noted otherwise, scalar speech-readout metrics from the outer-fold protocol are reported as mean ± s.d. across the five held-out test folds. Figure 4d,e summarize governed validation-wide neighborhood-contraction statistics derived from checkpoint replay. Figure 5a-c place stage-0 support, first-step contraction, final family locality, and exact-versus-local clean consequence on the same neighborhood-preservation logic. Figure 5d makes that hierarchy visible directly as four-family final confusion morphology. Figure 5f reports a family-to-measured local-band agreement score defined by within-15° retained mass with a secondary weighting from the rescaled correlation between the decoder’s per-angle local-band profile and the measured neighborhood profile; open-circle annotations on the same panel show the corresponding full-matrix correlation to the measured neighborhood as a secondary reference. Figure 5g reports the coherent five-seed babble SNR sweep with mean ± s.e.m. across sweep seeds. When statistical hypothesis tests are used, the test, sidedness, and multiple-comparison correction are stated alongside the corresponding figure panel.
