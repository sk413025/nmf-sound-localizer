...
Here we show that ordinary passive objects can themselves act as directional sensing substrates: at one fixed measurement point, their vibration response carries directional information.
...
Readout succeeds only when it preserves that broadened nearby-angle evidence long enough for subtraction to sharpen it.
Across acrylic, paper, cardboard, wood, and a laptop shell, the same directional code recurs, and cross-object recoverability is governed more by local separability than by response energy alone.
Directional sensing therefore need not be designed only into dedicated hardware: some directional front-end information can already reside in passive structures present in a scene.
...
That observation changes the decoding problem itself. Once the calibrated fingerprints are placed on the discrete angle grid, direction recovery is no longer a search over 37 unrelated references. Nearby calibrated directions can become jointly plausible before a final angle is chosen, so the key question becomes local separability rather than global matching. The next question is whether realistic source variation preserves that code or weakens that separability.
...
Once speech broadens overlap locally, the readout problem changes: what kind of update remains compatible with the measured directional code? Figures 4 and 5 show that the decisive condition is not decoder complexity but neighborhood preservation. Nearby-angle evidence has to remain coherent long enough for subtraction to sharpen it. If subtraction acts on an already collapsed support, the readout fractures the structure that still carries direction. If local support is preserved first, exact direction can re-emerge [@gregor2010lista; @monga2021unrolling].

Figure 4 enters the first routed step because that is the cleanest place to see this condition. The broad physical match is still organized around one nearby-angle neighborhood, so an admissible update must contract that existing support rather than replace it. The representative clip in Fig. 4b shows this local carve-out directly, and the validation-wide summaries in Fig. 4c-f show the same inward contraction across thresholds, angles, and clips. The result is the same across these views: broadened local support has to be preserved before subtraction is allowed to sharpen it.
...
b, A local update is carved from broad support. For the same representative clip, broad physical support, the learned cue, the routed weight profile, and the resulting update are plotted on one shared angle frame. The local update is carved from within the broad support rather than inventing a new neighborhood.
...
Figure 5 then asks whether that same condition still orders final prediction. It does. On the shared radius view, speech begins broad but local, the first update pulls that support inward, and final guided predictions stay concentrated near the correct angle (Fig. 5a). The family comparison keeps the same scientific question in view: which readout rules keep nearby-angle evidence local enough to sharpen, and which ones fracture it by committing too early? Families that preserve local support retain tight confusion structure and stronger exact prediction, whereas families that spread or break that support do not (Fig. 5b-d). The calibrated local neighborhood in Fig. 5e is the physical reference for that comparison, and Fig. 5f-g show that final performance follows how closely each family stays aligned with that measured geometry under clean and noisy conditions. Decoder performance is therefore ordered by whether the readout respects the measured local geometry.
...
Across five ordinary passive objects—an acrylic plate, paper cup, cardboard box, wooden board, and laptop shell—the same directional code recurs. Figure 6a-c show that these objects differ widely in structural character and in the width and compactness of their directional responses, yet each still produces structured angle-frequency fingerprints, a finite neighborhood of positive local ordering, and above-chance single-point readout under matched calibration. The cross-object question is therefore whether recoverability is still ordered by local separability once the object itself changes.

Figure 6d-e answers yes, but in a bounded way. Performance does not follow response energy alone. It falls when neighboring directions overlap more strongly, and it shifts with the object-specific band that carries the clearest directional contrast. The paper cup makes the contrast explicit: it has the strongest overall response energy in this set, yet its broader local overlap yields worse readout than cardboard. What recurs across objects is therefore not one universal spectrum but one recurring directional code expressed through different passive structures. Across this tested archetype set, passive structure is part of the sensing substrate because directional usability is ordered by the same local geometry across structurally different objects.
...
## Discussion
Directional sensing need not be designed only into arrays and specialized sensors. Across the tested passive-object archetypes, one fixed vibrometric point repeatedly records a recurring directional code with a finite local neighborhood after matched calibration. Passive structure is therefore part of the sensing front end, not merely a nuisance surrounding the sensor.

The same results also show what governs whether that code remains readable. Speech does not erase directional structure; it broadens it into structured local ambiguity. Readout succeeds when it preserves that nearby-angle evidence long enough for subtraction to sharpen it, and across objects the same locality principle orders recoverability more strongly than response energy alone. The broader claim is therefore specific rather than expansive: passive structures can already carry directional front-end information, and whether that information can be recovered depends on how distinctly the local geometry survives.

Placed in context, this finding links the paper to literatures that treat complex scattering as an encoding resource and to single-sensor localization studies that exploit direction-dependent transfer structure without large arrays [@jiang2020randomized_metamaterial; @hoang2021single_pixel_doa; @elbadawy2018lego_doa; @dipassio2023doa_single_sensor; @rutowski2024reverb_single_sensor; @rutowski2025_structural_beamforming]. The difference here is that the same logic appears in unmodified passive objects rather than in engineered structures or specialized directional hardware. The paper therefore shifts the role of passive structure itself: matched calibration does not merely compensate for nuisance scattering, it exposes directional organization already carried by the structure.
...
The readout analysis uses three aligned formal surfaces. The calibrated dictionary \(H\) is the measured map of angle-dependent fingerprints in the full standardized feature space. The reduced surrogate \((z,A)\) is a diagnostic view of nearby-angle competition used to expose hard commitment. The grouped dictionary \(D=[d_{e,m}]\) is the full-space readout object used by the implemented solver. These surfaces support one scientific question: once nearby calibrated directions overlap, what update preserves that measured local neighborhood long enough for subtraction to sharpen it?

The implemented solver operates on \(D\) without projecting the actual readout into the reduced SVD basis. The formulation rests on three working assumptions: (1) over each analysis window the target behaves approximately as a linear time-invariant system, so direction-dependent responses superpose in the frequency domain; (2) the calibrated dictionary \(H\) gives a sparse local approximation in standardized fingerprint space; and (3) discretizing the angle ordering supplies a useful single-source surrogate over nearby calibrated directions. Supplementary Methods 1 gives a representative Kirchhoff-Love plate operator and Green's function derivation for assumption (1).
...
where \(z\) is the reduced fingerprint, \(A\) is the reduced calibrated dictionary, \(x\) is a sparse coefficient vector, and \(K\) is a small residual-correction budget for local overlap and noise. This reduced view is not the deployed inference surface; it is the cleanest formal surface for showing why early hard commitment fails when nearby directions share structure.

Once direction is discretized onto the measured angle grid, hard OMP provides the classical greedy sparse-recovery baseline for this local-overlap surrogate. Supplementary Methods 3 gives the recursion explicitly, and Fig. 3 uses the corresponding pre-update grouped match as the diagnostic of exact versus local support before any residual refitting.

The guided solver keeps the same \(K\)-stage residual-correction scaffold but changes the commitment rule. Instead of collapsing support onto one direction immediately, it first pools evidence across one locally plausible neighborhood and only then applies subtraction. The residual is initialized as \(r_0 = \tilde y\) and the sparse coefficient state as \(x_0 = 0\). At each stage \(t = 1,\dots,K\), the residual is compared with the grouped dictionary to form the physical match score

$$g_t = D^\top r_t, \qquad (4)$$

where \(r_t\) is the current residual and \(g_t\) measures how strongly that residual aligns with the grouped calibrated fingerprints. A compact transformer encoder (embedding dimension \(d_\mathrm{model}=128\), 2 attention heads, 1 encoder layer in the reported primary configuration) then uses the same residual to produce a direction-level routing score

$$s_t[e] = \frac{\langle q_t, k_e \rangle}{\sqrt{d_k}}, \qquad (5)$$

where \(q_t\) is a learned query derived from the current residual, \(k_e\) is the learned key associated with direction group \(e\), and \(d_k\) is the key dimension. This score does not replace the physical match. It decides where that match should remain concentrated before subtraction, so the update stays tied to one locally plausible neighborhood. In the reported implementation, selection uses a straight-through Gumbel approximation in the forward pass, so the routing acts as a discrete local gate over neighboring angle hypotheses. The resulting gated update is written compactly as

$$\Delta x_t = w_t \odot g_t, \qquad (6)$$

where \(\odot\) denotes element-wise multiplication and the direction-level gate \(w_t\) is understood to broadcast across the grouped local match within each retained neighborhood. Supplementary Methods 4 writes that same grouped construction explicitly at atom resolution. The sparse coefficient state is then accumulated as \(x_{t+1} = x_t + \eta\,\Delta x_t\), where \(\eta\) is a learned step-size parameter, and the residual is corrected by dictionary-consistent subtraction:

$$r_{t+1} = r_t - D\,(\eta\,\Delta x_t), \qquad (7)$$

equivalently, \(r_{t+1} = \tilde y - D x_{t+1}\). In this form, the readout remains on the measured fingerprint surface while preserving nearby-angle support long enough for subtraction to sharpen it. Direction supervision and final prediction come from the same direction-level scores:
