...
Figure 5 follows that same neighborhood metric to the decision surface. On one shared radius axis, speech stage-0 matching carries little exact support but substantial local mass, the first guided step contracts that same mass sharply inward, and final guided predictions remain almost entirely inside the same neighborhood (Fig. 5a). The same metric then orders decoder families at final prediction: guided decoding preserves the most local mass, router-bypass remains partially local, the OMP baseline broadens further, and dense routing largely loses neighborhood concentration (Fig. 5b). That ordering is visible directly in the final-prediction locality maps, where guided predictions remain near the diagonal while the OMP baseline fractures into broader off-diagonal errors after committing too early (Fig. 5c). The measured neighborhood geometry and the guided neighborhood geometry then show why: the calibrated angle-angle correlation map and the guided score-surface correlation map retain the same coarse near-diagonal organization, and the corresponding full-matrix and local-band correlations exceed the 95th-percentile permutation nulls (Fig. 5d-f; \(r = 0.47\) globally and \(r = 0.46\) for the local-band summary). Performance follows the same hierarchy across angle and SNR (Fig. 5g). Final prediction succeeds here because the admissible readout keeps the measured neighborhood local all the way to the decision surface.
...
Top-1 accuracy is reported on the discrete angle grid. Angular error is the minimal angular difference \(\Delta(\hat\theta,\theta)=\min_{k\in\mathbb{Z}}|\hat\theta-\theta+360k|\), and root-mean-square error (RMSE) = \(\sqrt{\frac{1}{N}\sum_i \Delta(\hat\theta_i,\theta_i)^2}\). Unless noted otherwise, scalar speech-readout metrics from the outer-fold protocol are reported as mean ± s.d. across the five held-out test folds. Figure 4d,e summarize governed validation-wide neighborhood-contraction statistics derived from checkpoint replay. Figure 5a-b place stage-0 support, first-step contraction, and final confusion locality on the same radius-based neighborhood metric, Fig. 5c-e are clean representative locality and geometry displays, Fig. 5f gives the neighborhood-alignment summary, and Fig. 5g stacks clean and SNR-sweep evaluation summaries with mean ± s.e.m. across sweep seeds. When statistical hypothesis tests are used, the test, sidedness, and multiple-comparison correction are stated alongside the corresponding figure panel.
