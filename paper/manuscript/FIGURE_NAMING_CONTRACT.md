...
- Treat Fig. 4 as a six-panel admissibility pivot: panel `a` is a high-information admissibility synthesis that combines validation-mean broad-versus-contracted support with one representative 70° clip on the same local frame; panel `b` shows how the routed local update is carved from within the broad physical support; panel `c` compresses first-step recovery onto exact, 5°, 10°, and 15° operating points; panel `d` gives the validation-wide contraction curve; panel `e` fixes the operative 15° neighborhood to show the same inward shift across the angle grid; and panel `f` shows that the 15° gain is positive for nearly all clips.
