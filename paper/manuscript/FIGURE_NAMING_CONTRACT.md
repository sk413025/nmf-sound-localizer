...
- Treat Fig. 4 as a six-panel admissibility pivot: panel `a` defines admissible contraction against the measured neighborhood on one shared angle frame; panels `b-c` show a representative broad-to-local transition before and after the first guided step; panel `d` gives the validation-wide contraction curve; panel `e` fixes the operative 15° neighborhood to show the same inward shift across the angle grid; and panel `f` shows that the 15° gain is positive for nearly all clips.
