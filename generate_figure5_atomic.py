#!/usr/bin/env python3
"""
Generate Figure 5 Atomic Panels (Nature Style)
Based on logic from nature-comm-repro commit bab0cf2
"""
import numpy as np
...
def create_atomic_panels_a(routing_data, dict_data, out_dir):
    ...
    # Size: ~45mm wide (half column)
    fig = na_style.make_figure(width_mm=45, height_mm=40)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(H_corr, cmap='viridis', aspect='auto', vmin=-1.0, vmax=1.0)
    ...
    # Colorbar needs careful handling in small figures
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=40)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(expert_corr, cmap='viridis', aspect='auto', vmin=-1.0, vmax=1.0)
...
def create_atomic_panels_b(routing_data, dict_data, out_dir):
    ...
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(physics_true_heatmap, aspect='auto', cmap='viridis',
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=35)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(physics_wrong_heatmap, aspect='auto', cmap='viridis',
                   extent=[0, 8, 2000, 300], interpolation='nearest',
                   vmin=vmin_phys, vmax=vmax_phys)
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=35)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(qk_true_heatmap, aspect='auto', cmap='RdYlGn',
                   extent=[0, 8, 2000, 300], interpolation='nearest',
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=35)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(qk_wrong_heatmap, aspect='auto', cmap='RdYlGn',
                   extent=[0, 8, 2000, 300], interpolation='nearest',
                   vmin=vmin_qk, vmax=vmax_qk)
    ...
    # Legend might be too big for this small plot, adjust
    ax.legend(fontsize=5, loc='upper right', bbox_to_anchor=(1.4, 1.1))
    ax.set_ylim(0, 1.1)
...
def create_atomic_panels_c(routing_data, out_dir):
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=40)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(physics_prob, cmap='hot', aspect='auto',
    ...
    fig = na_style.make_figure(width_mm=45, height_mm=40)
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(qk_prob, cmap='hot', aspect='auto',
                   extent=[0, 37, 37, 0], vmin=0, vmax=vmax_unified,
                   interpolation='nearest')
    ax.set_ylabel('True DOA (θ°)')
    ax.set_xlabel('Selected Expert')
...
def main():
    ...
    out_dir = Path("results_figure5_v1")
    out_dir.mkdir(exist_ok=True, parents=True)
    
    print("="*60)
    print("Generating Figure 5 Atomic Panels (Nature Style)")
    print("="*60)
