...
- Run: `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/`
- Mode: `full_dataset`
- Outcome: `PASS`
...
- `code_state.json`: `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/code_state.json`
...
- Subset manifest: `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/subset_manifest.json`
  - `num_pairs = 416`
...
OUT_DIR="results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619"
...
From `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/summary/compute_matched_summary.json`:
...
- capture_min/max per method:
  ...
  - Random: min `0.0`, max `0.9999999404`
...
From `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/summary/rtg_controllability_summary.json`:

- spearman(lambda_c, k_selected_mean) = `-1.0` (PASS)
- k_selected_range = `0.7087323` (non-degenerate)
- Distributional metrics:
  - P(k_selected < max_k) at min/max lambda: `0.0040937` -> `0.1715173`
...
- DT_capture_mean = `0.9987098`
- OMP_capture_mean = `0.9995393`
- Random_capture_mean = `0.9973956`
- DT/OMP = `0.9991701`
- DT - Random = `+0.0013142`
- k_selected_mean = `15.9631`

(For context) At lambda_c = `1e-2` (max penalty):
- DT_capture_mean = `0.9966024`
- OMP_capture_mean = `0.9987358`
- Random_capture_mean = `0.9872327`
- DT/OMP = `0.9978639`
- DT - Random = `+0.0093696`
- k_selected_mean = `15.2544`
...
From `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/summary/forced_k_summary.json`:

- K=1: DT `0.6716845`, OMP `0.6944392`, Random `0.1762305`, DT/OMP `0.9672330`, DT-Random `0.4954540`
- K=2: DT `0.7651018`, OMP `0.8463252`, Random `0.3349198`, DT/OMP `0.9040281`, DT-Random `0.4301820`
- K=4: DT `0.8602174`, OMP `0.9528764`, Random `0.6018571`, DT/OMP `0.9027587`, DT-Random `0.2583603`
- K=8: DT `0.9709198`, OMP `0.9939977`, Random `0.9232208`, DT/OMP `0.9767828`, DT-Random `0.0476990`
- K=16: DT `0.9993430`, OMP `0.9998525`, Random `0.9986362`, DT/OMP `0.9994904`, DT-Random `0.0007068`

## 5) Interpretation (Required; causal language)

- RTG controllability is visible primarily in the stop-rate tail BECAUSE most samples still saturate at max_k=16, THEREFORE mean k_selected moves modestly while P(k_selected < max_k) increases with lambda.
...
- DT remains close to OMP under both compute-matched and forced-K tables BECAUSE the learned lag-selection policy approximates greedy pursuit behavior on this in-domain speech dataset.

## 6) Failures (Required even if PASS)

- No hard failures observed in this full-dataset run.
- Note: `results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619/integrity_diagnostics.jsonl` is empty BECAUSE no out-of-range capture events occurred.
...
# 2) Execution
OUT_DIR="results/rtgomp_lambda_cost_E4h_speech_paper_eval_full_dataset_20260123_210619"
conda run --no-capture-output -n trl-training python scripts/h_exploration/run_rtgomp_e4h_paper_eval.py   --mic_root /Users/jnrle/Documents/LDVReorientation/data/SpeechData/boy1/MIC   --ldv_root /Users/jnrle/Documents/LDVReorientation/data/SpeechData/boy1/LDV   --ckpt_path results/rtgomp_lambda_cost_E4c_stopstatefix_ratio3_20260121_030820/model/dt_freq_aware_best.pth   --out_dir "$OUT_DIR"   --mode full_dataset   --hop_length 160 --fs 16000 --n_fft 2048 --freq_min 300 --freq_max 3000   --max_lag 50 --max_k 16 --tw 32 --gain 100.0 --rtg_dim 2   --lambda_c_values "1e-4,3e-4,1e-3,3e-3,1e-2"   --random_trials 1 --random_sampling without_replacement --seed 0   --write_per_sample 0 --device cpu

# 3) Verification
# Expected:
# - integrity.num_capture_out_of_range_total == 0
# - summary/compute_matched_summary.json exists
# - DT - Random at lambda=1e-4 is positive
```

## 8) Next Steps (Required)

- Use these full-dataset speech numbers for paper tables/figures.
- If stronger controllability (larger mean shift) is desired, consider widening the lambda grid or re-training STOP calibration BECAUSE the distribution is still saturated near max_k.
