...
- `figure5_b3_v3_vs_v10_report.md` shows that OMP agrees but QK does not, because the definitions differ.
- `polar_plot_architecture_analysis.md` explains why `scores_expert` is not frequency-separable and why any banded QK score is necessarily indirect/proxy unless you use attribution.
...
Reference: `scripts/omp-transformer.py:192`–`scripts/omp-transformer.py:196` and the architectural notes in `polar_plot_architecture_analysis.md`.
