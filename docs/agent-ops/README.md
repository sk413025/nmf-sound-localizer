...
3. when delegating, the top-level supervisor chooses the child role, specialist skill, and unified task packet
4. the round records `Relevant conversation context`, `Context mode`, risk level, and ownership boundaries
5. for manuscript-facing hardening, the top-level supervisor selects the applicable reviewer roles and evaluation goals from `NATURE_REVIEWER_STACK.md`
6. direct work or delegated child work proceeds through one of the core skills
...
8. the packet or closeout classifies the round as `high-risk` or `non-high-risk`
9. for `high-risk` rounds, the round names both a review owner and a verification owner; compressed ownership requires an explicit compression rationale
10. for `non-high-risk` rounds, compressed ownership requires a non-high-risk rationale
11. the top-level supervisor consolidates only against the round's owned plan items, acceptance surface, delivery evidence, and verification target
12. if delivered scope is narrower than planned scope, the top-level supervisor discloses the downgrade explicitly rather than closing out the original round as complete
