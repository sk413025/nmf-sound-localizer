...
## What This Branch Is Currently Optimizing For
...
- the paper must produce one explicit cognitive shift from an `old-world belief` to a `new-world belief`
...
- treat sentence polish as insufficient when protagonist, pivot, or tool-vs-discovery weighting is still wrong
...
- keep the `editor-first` reading contract explicit: a skimming Nature Communications editor should be able to tell, without `Methods`-first decoding, what physical phenomenon is in play, what observable or quantity carries it in the current paragraph, and why it matters
- keep scope control affirmative: narrow the supported claim itself, then add the real boundary; do not let `not X`, `should not be read as`, or other reviewer-prebuttal phrasing leak into paper-facing prose
- keep `Results-to-method` linkage quantity-first, equation-backgrounded, surface-explicit, and vocabulary-light: Results should state what the quantity does and why it matters while full formulas stay in Methods, and umbrella terms such as local neighborhood or directional code must stay tied to the actual analysis surface they name without forcing readers to learn a private term map
- keep bidirectional no-translation traceability explicit through single-head term propagation: a reader should be able to move from `Introduction/Results` into `Methods` and from `Methods/Supplementary` back into the paper-facing argument while recognizing the same scientific actors under one canonical head term rather than building a second term dictionary
- keep `Methods` / `Supplementary Methods` first-mention lock explicit: when a formal surface reopens a concept family, start with the same paper-facing head term used in the main text, then attach notation, equation labels, or exact score names as added precision
- keep legend parity explicit: figure legends should reinforce the same canonical head terms used in the surrounding manuscript rather than behaving like a second naming surface
...
- turning scope control into reviewer-prebuttal prose; if a sentence starts explaining how it should not be misread, the claim should usually be narrowed and rewritten instead
...
- letting noun stacks, internal labels, or formal score names replace explicit cause-effect explanation, forcing readers to learn a private term map before they can follow the scientific inference
- accepting one-way method traceability as enough; if readers can defend a Results term from Methods but cannot naturally map the formal Methods object back to the same paper-facing concept, the manuscript still has vocabulary drift
- accepting concept-only alignment as enough; if a reader must map several near-synonyms back onto one concept family, the naming has not yet landed even when the science is aligned
...
- archive note: [2026-04-13 Nature Comm Reframe](./archive/2026-04-13-nature-comm-reframe.md)
