#!/usr/bin/env python3
"""
Generate Figure 5 Atomic Panels (Nature Style) - v6
Specific Dimensions and Consistent Alignment
"""
import numpy as np
...
na_style.set_nature_rcparams(base_fontsize=7) # Slightly smaller font for small panels
plt.rcParams.update({
    "font.family": "serif",
...
def apply_consistent_layout(fig, width_mm, height_mm, is_polar=False):
    """
    Apply consistent margins to align text with upper and lower bounds.
    Margins are defined in mm to ensure consistency across different figure sizes.
    """
    # Margins in mm
    # Top: Space for Title
    # Bottom: Space for X-label and Ticks
    # Left: Space for Y-label and Ticks
    # Right: Space for Colorbar or Legend
    
    if is_polar:
        # Polar plot needs different margins
        top_mm = 6.0
        bottom_mm = 2.0 # Less space needed at bottom for polar usually
        left_mm = 2.0
        right_mm = 2.0
        # For B3 specifically, we want the title to align with the top of the other plots
        # and the bottom to align with the bottom of the other plots (when stacked).
        # But B3 is 102mm high, others are 50mm.
        # If we assume B3 spans two rows of 50mm plots, 
        # Top of B3 should align with Top of Row 1.
        # Bottom of B3 should align with Bottom of Row 2.
        # So margins should be similar to the standard plots.
        top_mm = 5.0
        bottom_mm = 8.0
        left_mm = 4.0
        right_mm = 4.0
        ...
        bottom_mm = 8.0
        left_mm = 8.0
        right_mm = 9.0 # Space for colorbar

    # Convert to relative coordinates (0-1)
    top = 1.0 - (top_mm / height_mm)
    bottom = bottom_mm / height_mm
    left = left_mm / width_mm
    right = 1.0 - (right_mm / width_mm)
...
def create_atomic_panels_a(routing_data, dict_data, out_dir):
    ...
    fig = na_style.make_figure(width_mm=W_STD, height_mm=H_STD)
    ...
    apply_consistent_layout(fig, W_STD, H_STD)
    
    # Colorbar needs to be added AFTER layout adjustment or carefully managed
    # But subplots_adjust affects the axes position.
    # Let's add colorbar to the axes.
...
def create_atomic_panels_b(routing_data, dict_data, out_dir):
    ...
    # B3: Polar Plot - REFINED V6 (Right Half Only, Specific Dimensions)
    fig = na_style.make_figure(width_mm=W_POLAR, height_mm=H_POLAR)
    ...
    # Restrict to Right Half (0 to 180 degrees)
    ax.set_thetamin(0)
    ax.set_thetamax(180)
    ...
    # Adjusted position for the new aspect ratio
    ...
    apply_consistent_layout(fig, W_POLAR, H_POLAR, is_polar=True)
...
def main():
    ...
    # New output directory structure: nature-figures/results/results_figure5_v6
    out_dir = Path("results/results_figure5_v6")
    out_dir.mkdir(exist_ok=True, parents=True)
    
    print("="*60)
    print("Generating Figure 5 Atomic Panels (Nature Style) - v6")
    print("="*60)
