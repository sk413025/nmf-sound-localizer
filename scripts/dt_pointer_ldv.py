...
def main():
    ...
    # Train/Test split (by sample, not by step)
    print(f"\n{'='*80}\nTrain/Test Split\n{'='*80}")
    n_samples = len(samples)
    n_test = max(1, int(n_samples * args.test_split))
    n_train = n_samples - n_test
    
    # Stratified split: ensure each angle is represented in both sets
    ...
    # Split each angle group proportionally
    ...
    # Training loop with train/test monitoring
    print(f"\n{'='*80}\nTraining Start\n{'='*80}")
    ...
    for epoch in range(args.epochs):
        ...
        if test_loss < best_test_loss:
            ...
            try:
                ...
                print(f"[Best checkpoint] Save failed: {e}")
        ...
        # Record history
        hist.append({
            'epoch': (hist[-1]['epoch'] + 1 if hist else 1),
            'train_loss': train_loss,
            'train_loss_eval': train_loss_eval,
            'train_acc_e': train_acc_e,
    ...
    # Final evaluation on full train set (legacy output for compatibility)
    model.eval()
    with torch.no_grad():
