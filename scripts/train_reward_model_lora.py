...
def _prepare_samples(args, direction_angles: List[int]) -> Tuple[List[str], Dict[str, Dict[str, torch.Tensor]]]:
    ...
    if args.use_multi_modal:
        ...
        # Load H matrix for direction projection tokenizer
        ...
        print(f"  ✓ Multi-modal: H matrix loaded {H.shape}, angles: {h_angles_list}")
        ...
        print(f"  ✓ Multi-modal: PromptBuilder created ({args.token_ordering}, max_tokens={args.max_tokens})")
        ...
        print(f"  ✓ Multi-modal: DoAICLDataset created ({len(ds)} samples)")
...
def main():
    ...
    if args.use_multi_modal:
        ...
        print(f"  - Top-k atoms: {args.top_k_atoms}")
        print(f"  - Top-m directions: {args.top_m_directions}")
        if args.icl_mode:
