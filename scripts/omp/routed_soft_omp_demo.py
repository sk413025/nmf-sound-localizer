...
class TrainableRoutedSoftOMP(nn.Module):
    ...
    def forward(self, y: torch.Tensor, D: torch.Tensor, train_mode: bool=True):
        ...
        for _ in range(self.steps):
            ...
            # Scaled dot to align with attention spec (1/sqrt(d))
            scale = 1.0 / math.sqrt(float(D_emb.size(0)))
            scores_atoms = (K * q.view(-1,1)).sum(dim=0) * scale   # [P]
            ...
            if train_mode:
                ...
                # hard routing using correlation-based selection per spec:
                # 1) compute correlations g = D^T r
                g = D.T @ r  # [P]
                G = g.view(self.E, self.M)  # [E, M]
                # 2) group score alpha_e = max_j in group |g_j|
                alpha = G.abs().max(dim=1).values  # [E]
                kE = min(self.top_e, self.E)
                chosen_e = torch.topk(alpha, k=kE).indices.tolist()
                # 3) within chosen groups, select top-L by |g|
                idx = []
                for e in chosen_e:
                ...
                # 4) sequential coordinate descent updates to ensure monotonic residual decrease
                #    for each selected index i (sorted by |g_i|), do x_i += g_i; r <- r - d_i * g_i
                if len(idx) > 0:
                    # sort by current |g_i| descending
                    idx = list(dict.fromkeys(idx))
                    idx.sort(key=lambda i: float(g[i].abs()), reverse=True)
                    for i in idx:
                        gi = (D[:, i] @ r)  # recompute with current residual
                        # columns of D are unit-norm; optimal coordinate-descent step is gi
                        x[i] = x[i] + gi
                        r = r - D[:, i] * gi
            r = y - D @ x
...
@torch.no_grad()
def hard_infer_once(model: TrainableRoutedSoftOMP, D: torch.Tensor, D_true: torch.Tensor, idx2_true: List[Tuple[int,int]],
                    device='cpu'):
    model.eval()
    yb, xb = synth_batch(D_true.to(device), idx2_true, B=1)
    x_hat, r_curve = model(yb[0].to(device), D.to(device), train_mode=False)
    # Group energy for DoA proxy
    E = model.E; M = model.M
    A_group = x_hat.view(E, M).abs().sum(dim=1).cpu().numpy()
    doa_pred_idx = int(np.argmax(A_group))
    return x_hat, np.array(r_curve), A_group, doa_pred_idx
...
def main():
    ...
    # Single-sample hard inference for DoA proxy
    _, r_curve_sample, A_group, doa_pred_idx = hard_infer_once(model, D_id, D_id, idx2_id, device=device)
    ...
    axs[3].bar(np.arange(len(A_group)), A_group)
    axs[3].axvline(doa_pred_idx, color='r', linestyle='--', label=f'pred={doa_pred_idx}')
    axs[3].set_title("Group energy A_group")
    axs[3].set_xlabel("group (expert) index")
    axs[3].legend()
    if args.out_dir:
        ...
        np.savez(os.path.join(args.out_dir, "metrics.npz"),
                 ...
                 r_curve_sample=np.array(r_curve_sample),
